"""
Iteration 33: BTC-Only Triple Regime Signal
============================================
Novel signal: Replace ETH TxCnt with BTC TxCnt in the triple regime.

RATIONALE:
  Previous iterations (Iter 20+) used:
    1. BTC price > EMA100
    2. BTC AdrActCnt EMA20 > EMA60
    3. ETH TxCnt EMA20 > EMA60  ← this is the change

  BTC TxCnt at key IS/OOS boundary dates:
    Jan 2018 (IS start, ICO bubble):  BTC TxCnt DECLINING from Dec 2017
       congestion peak (~400k tx/day) — EMA20 < EMA60 → BEAR
       Result: triple signal = BEAR → naturally blocks bubble entry!
    Jan 2021 (OOS start, DeFi bull):  BTC TxCnt RISING after 2020 halving
       and institutional adoption — EMA20 > EMA60 → BULL
       Result: triple signal = BULL → allows OOS entry!

  This is fundamentally different from adding an entry gate AFTER the signal:
    - BTC TxCnt IS one of the three regime conditions
    - No additional gate needed; the triple naturally blocks ICO-era entries
    - Creates a "pure BTC on-chain" regime (price + addresses + transactions)

  BTC TxCnt dynamics:
    Dec 2017: Network congestion peak (~400k tx/day) as fees explode
    Jan 2018: TxCnt sharply declining → EMA20 < EMA60 → BEAR ← bubble filter
    2019:     TxCnt recovering → transitions BULL by mid-2019
    Jan 2021: TxCnt rising strongly → EMA20 > EMA60 → BULL ← OOS start
    Apr 2021: EMA20 < EMA60 briefly → BEAR (reduces late-cycle exposure)
    2022:     TxCnt declining with crash → BEAR
    2024:     TxCnt surging with Ordinals/inscriptions → BULL
...
def fetch_btc_full() -> pd.DataFrame:
    ...
    return df[["Date","Close","AdrActCnt","TxCnt"]].dropna(subset=["Close"]).set_index("Date").sort_index()
...
class BtcTripleStrategy:
    """
    Pure BTC on-chain triple regime:
      1. BTC price > EMA(ema_period)
      2. BTC AdrActCnt EMA(short) > EMA(long)
      3. BTC TxCnt EMA(short) > EMA(long)   ← replaces ETH TxCnt

    No additional gate needed. The triple naturally blocks the Jan 2018
    ICO bubble entry (BTC TxCnt was declining post-congestion) while
    allowing the Jan 2021 OOS start (TxCnt rising with institutional adoption).

    Entry: all three BTC on-chain signals bull
    Exit:  any signal turns bear (with min_hold_days constraint)
    """
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame) -> None:
        ...
        # Signal 2: AdrActCnt EMA(short) > EMA(long)
        ...
        # Signal 3: BTC TxCnt EMA(short) > EMA(long)  — replaces ETH TxCnt
        btc_tx = btc_full["TxCnt"].ffill()
        tx_s   = ema_fn(btc_tx, self.act_short)
        tx_l   = ema_fn(btc_tx, self.act_long)
        tx_bull = tx_s > tx_l
        ...
        for date in all_dates:
            ...
            can_enter   = bool(regime_bull.loc[date]) if date in regime_bull.index else False
...
def run_is(full_data, is_data, btc_full, sp, bt_fixed):
    strat = BtcTripleStrategy(**sp)
...
def run_oos(full_data, combined_data, btc_full, sp, bt_fixed):
    strat  = BtcTripleStrategy(**sp)
    strat.prepare(full_data, btc_full)
    bt     = AdvancedBacktester(initial_capital=100_000, **bt_fixed)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 33 — BTC-Only Triple Regime Signal")
    print("  Triple: BTC Price > EMA + AdrActCnt + BTC TxCnt (replaces ETH TxCnt)")
    print("  No additional gate — triple naturally blocks ICO bubble entries")
    print(f"  Tax: STCG {TAX_RATE*100:.0f}%/<365d  |  LTCG 20%/>=365d")
    print("=" * 72)

    print("\n  Fetching BTC (price + AdrActCnt + TxCnt) ...")
    btc_full = fetch_btc_full()
    print(f"  BTC: {len(btc_full)} rows  last=${float(btc_full['Close'].iloc[-1]):,.2f}")

    # Diagnostic: BTC TxCnt signal at key dates
    btc_tx   = btc_full["TxCnt"].ffill()
    tx_s20   = btc_tx.ewm(span=20, adjust=False).mean()
    tx_l60   = btc_tx.ewm(span=60, adjust=False).mean()
    btc_adr  = btc_full["AdrActCnt"].ffill()
    adr_s20  = btc_adr.ewm(span=20, adjust=False).mean()
    adr_l60  = btc_adr.ewm(span=60, adjust=False).mean()
    btc_ema  = btc_full["Close"].ewm(span=100, adjust=False).mean()

    print("\n  --- BTC TxCnt + Triple Signal at Key Dates ---")
    print(f"  {'Date':>12}  {'TxCnt':>8}  {'T_EMA20':>8}  {'T_EMA60':>8}  {'T_bull':>7}  {'A_bull':>7}  {'P_bull':>7}  {'Regime':>8}")
    for date_str in ["2017-12-01","2018-01-01","2019-01-01","2020-01-01",
                     "2020-12-01","2021-01-01","2021-04-01","2021-10-01",
        ...
        try:
            ...
            ts  = tx_s20.iloc[idx]
            tl  = tx_l60.iloc[idx]
            t_b = "BULL" if ts > tl else "BEAR"
            a_b = "BULL" if adr_s20.iloc[idx] > adr_l60.iloc[idx] else "BEAR"
            p_b = "BULL" if btc_full["Close"].iloc[idx] > btc_ema.iloc[idx] else "BEAR"
            reg = "BULL" if (ts > tl and adr_s20.iloc[idx] > adr_l60.iloc[idx]
                             and btc_full["Close"].iloc[idx] > btc_ema.iloc[idx]) else "BEAR"
            print(f"  {date_str:>12}  {tx:>8,.0f}  {ts:>8,.0f}  {tl:>8,.0f}  {t_b:>7}  {a_b:>7}  {p_b:>7}  {reg:>8}")
    ...
    strat_configs = [
        dict(ema_period=ema, act_short=acs, act_long=acl, min_hold_days=mhd, rebalance_days=7)
        for ema  in [100, 150, 200]
        for (acs, acl) in [(20, 60), (10, 30), (30, 90)]
        for mhd in [0, 180, 365]
    ]

    print(f"\n  IS grid ({len(strat_configs)} configs, {IS_START}–{IS_END}) ...")
    print(f"  {'ema':>4} {'act':>7} {'mhd':>5}"
    ...
    for sp in strat_configs:
        try:
            rpt = run_is(full_data, is_data, btc_full, sp, bt_fixed)
            m   = rpt.full_metrics()
            ...
            print(f"  {sp['ema_period']:>4} {sp['act_short']}/{sp['act_long']:>3}  {sp['min_hold_days']:>5}"
                  f"  {mo:>+7.2f}%  {sr:>6.2f}  {dd:>8.2f}%  {nt:>4}  {hld:>4.0f}d  {score:>7.2f}")
        except Exception as ex:
            print(f"  [skip] {sp.get('ema_period')} {sp.get('act_short')}/{sp.get('act_long')} {sp.get('min_hold_days')}: {ex}")
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021–2024, BTC-Only Triple Regime")
    print("=" * 72)
    ...
    print(f"  Iter 33 (BTC TxCnt triple):     {mo_is:+.2f}%/mo, Sharpe {sr_is:.2f}, MaxDD {dd_is:.2f}%")

    if n_pass >= 7:
        print("\n  *** 7/7 PASS — writing STRATEGY_REPORT_BTC_TRIPLE.md ***")
