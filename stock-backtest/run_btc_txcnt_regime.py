"""
Iteration 45: BTC TxCnt Regime + Vol Gate — 5-Coin Portfolio
=============================================================
Innovation: Replace ETH TxCnt (third regime signal) with BTC TxCnt.
Result: Triple regime becomes PURE BTC on-chain (no ETH dependency).

RATIONALE:
  Prior strategies (Iter 27/41): regime = BTC price + BTC AdrActCnt + ETH TxCnt
  This iteration:                 regime = BTC price + BTC AdrActCnt + BTC TxCnt

  Why BTC TxCnt might be better than ETH TxCnt for regime detection:
  - 2023: BTC TxCnt surged +200% from Ordinals/Runes inscriptions
    (ETH TxCnt was flat/declining as L2s absorbed mainnet activity)
  - BTC TxCnt captures BITCOIN network health directly, the primary asset
  - BTC AdrActCnt + BTC TxCnt together confirm broad Bitcoin network utilization

  2023-2024 CONTEXT:
    BTC TxCnt 2023: +200%+ surge (Ordinals boom → BRC-20 tokens → Runes)
    ETH TxCnt 2023: mixed (L2 migration reduced mainnet activity)
    → BTC TxCnt gives cleaner 2023-2024 OOS entry signal for the 2024 bull

KEY DATES — BTC TxCnt signal check:
  Jan 2018: TxCnt declining (mempool clearing after ICO bubble) → BLOCK ✓
  2018-2019: TxCnt declining in bear market → BLOCK ✓
  Oct 2020: TxCnt rising (accumulation phase) → ALLOW ✓
  Jan 2021: TxCnt rising → ALLOW ✓
  Jun 2021: China ban → TxCnt drops → BLOCK ✓
  2023-2024: TxCnt surging with Ordinals → ALLOW ✓ (earlier entry than ETH regime)

SIGNAL:
  Entry: regime_bull AND btc_vol30 < vol_threshold
  Exit:  regime_bull = False (vol does NOT trigger exits — preserves LTCG)

  regime_bull now = BTC price > EMA AND BTC AdrActCnt rising AND BTC TxCnt rising

Portfolio: BTC, ETH, BNB, ADA, TRX (5-coin same as Iter 41)
...
class BTCTxCntRegimeStrategy:
    """
    Pure BTC triple regime (price + AdrActCnt + TxCnt) with BTC vol gate.
    Entry: regime_bull AND btc_vol30 < vol_threshold
    Exit:  regime bearish (vol does NOT trigger exit — preserves LTCG)
    """
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        # Regime: BTC TxCnt EMA momentum (REPLACES ETH TxCnt)
        btc_txcnt  = btc_full["TxCnt"].ffill()
        tx_s       = ema_fn(btc_txcnt, self.act_short)
        tx_l       = ema_fn(btc_txcnt, self.act_long)
        tx_bull    = tx_s > tx_l
...
def run_is(full_data, is_data, btc_full, eth_full, sp, bt_fixed):
    strat = BTCTxCntRegimeStrategy(**sp)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 45 — BTC TxCnt Regime + Vol Gate (5-Coin)")
    print("  Regime: BTC price + BTC AdrActCnt + BTC TxCnt (replaces ETH TxCnt)")
    ...
    print(f"  ETH: {len(eth_full)} rows  last=${float(eth_full['Close'].iloc[-1]):,.2f}  [portfolio only]")

    # BTC TxCnt diagnostics
    btc_txcnt = btc_full["TxCnt"].ffill()
    btc_ret   = btc_full["Close"].pct_change()
    vol30     = btc_ret.rolling(30).std() * np.sqrt(252)
    tx_s20    = ema_fn(btc_txcnt, 20)
    tx_l60    = ema_fn(btc_txcnt, 60)
    print("\n  BTC TxCnt signal at key dates (EMA20/60):")
    for date_str in ["2018-01-01", "2020-09-01", "2021-01-01",
                     "2021-06-01", "2022-01-01", "2023-06-01", "2024-01-01"]:
        try:
            dt = pd.Timestamp(date_str)
            idx = btc_full.index.asof(dt)
            ts  = float(tx_s20.loc[idx])
            tl  = float(tx_l60.loc[idx])
            vl  = float(vol30.loc[idx]) if idx in vol30.index else float('nan')
            state = "RISING ✓" if ts > tl else "FALLING ✗"
            print(f"  {date_str}: TxCnt EMA {ts:,.0f} vs {tl:,.0f} → {state}  vol={vl:.2f}")
        except Exception:
            pass
    ...
    champ_rpt.print_full_report(
        f"OOS CHAMPION — BTC TxCnt Regime + Vol Gate 5-Coin"
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021–2024, BTC TxCnt Regime + Vol Gate (5-Coin)")
    print("=" * 72)
