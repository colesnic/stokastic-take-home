"""
Iteration 26: Combined ETH/BTC Rotation + MVRV Entry Gate
==========================================================
Combines two novel signals from prior iterations:
  - Iter 24: ETH/BTC ratio rotation (alt-season vs BTC-season filter)
  - Iter 25: ETH MVRV < threshold entry gate (valuation-aware entries)

STRUCTURE:
  BTC:  enter when main regime bullish
  ALTs: enter ONLY when regime_bull AND ETH/BTC uptrend AND MVRV < threshold
  All:  exit when main regime bearish (ETH/BTC and MVRV do NOT trigger exits)

HYPOTHESIS: combining alt-season timing (Iter 24) with valuation gating (Iter 25)
produces the highest-conviction alt entries — only when momentum AND valuation align.
...
class CombinedRotationStrategy:
    """
    Triple regime signal + ETH/BTC rotation (alt filter) + ETH MVRV entry gate.
    BTC:  enter when main regime bullish.
    ALTs: enter when main regime bullish AND ETH/BTC uptrend AND MVRV < threshold.
    Exit: main regime bearish only (ETH/BTC and MVRV do NOT trigger exits).
    """
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        # Main regime: BTC price + AdrActCnt + ETH TxCnt
        ...
        # ETH/BTC ratio trend (alt-season filter, entry-only)
        ...
        ethbtc_bull = (ethbtc_s > ethbtc_l).fillna(False)

        # MVRV entry gate (entry-only)
        eth_mvrv = eth_full["CapMVRVCur"].ffill()
        mvrv_ok  = (eth_mvrv < self.mvrv_threshold).reindex(btc_close.index).ffill().fillna(True)

        # Per-ticker entry conditions
        btc_entry = regime_bull                          # BTC: regime only
        alt_entry = regime_bull & ethbtc_bull & mvrv_ok  # ALTs: all three
        ...
        for date in all_dates:
            ...
            can_enter_btc = bool(btc_entry.loc[date]) if date in btc_entry.index else False
            can_enter_alt = bool(alt_entry.loc[date]) if date in alt_entry.index else False
            ...
            if should_exit:
                ...
                for _, t, _ in COINS:
                    ...
                    can_enter = can_enter_alt if is_alt else can_enter_btc
                    if can_enter:
                        if date in sig_dict[t].index:
                            sig_dict[t].loc[date] = 1
                        in_position[t] = date
...
def run_is(full_data, is_data, btc_full, eth_full, sp, bt_fixed):
    strat = CombinedRotationStrategy(**sp)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 26 — Combined ETH/BTC Rotation + MVRV Entry Gate")
    print("  ALT entry: regime_bull AND ethbtc_bull AND mvrv < threshold")
    print("  BTC entry: regime_bull only  |  Exit: regime_bull fails")
    ...
    # Diagnostics
    eth_btc_ratio = (eth_full["Close"] / btc_full["Close"].reindex(eth_full.index)).dropna()
    mvrv          = eth_full["CapMVRVCur"].dropna()
    eb20 = eth_btc_ratio.ewm(span=20, adjust=False).mean()
    eb60 = eth_btc_ratio.ewm(span=60, adjust=False).mean()
    ethbtc_bull = eb20 > eb60

    print("\n  ETH/BTC bull% and MVRV by year:")
    print(f"  {'Year':>4}  {'ETH/BTC Bull%':>14}  {'MVRV Min':>9}  {'MVRV Max':>9}  {'MVRV Start':>10}")
    for yr in range(2018, 2025):
        eb_sub = ethbtc_bull[ethbtc_bull.index.year == yr]
        mv_sub = mvrv[mvrv.index.year == yr]
        if len(eb_sub) == 0:
            continue
        print(f"  {yr:>4}  {eb_sub.mean()*100:>13.1f}%  {mv_sub.min():>9.2f}  {mv_sub.max():>9.2f}  {mv_sub.iloc[0]:>10.2f}")
    ...
    strat_configs = [
        dict(ema_period=ema, act_short=as_, act_long=al,
             ...
             mvrv_threshold=mv, min_hold_days=mhd, rebalance_days=7)
        for ema in [100, 150]
    ...
    print(f"  {'ema':>4} {'act':>7} {'ethbtc':>8} {'mvrv':>5} {'mhd':>4}"
    ...
    for sp in strat_configs:
        try:
            ...
            print(f"  {sp['ema_period']:>4} {sp['act_short']}/{sp['act_long']:>3}"
                  f" {sp['ethbtc_short']}/{sp['ethbtc_long']:>3}"
                  f" <{sp['mvrv_threshold']:.1f} {sp.get('min_hold_days',0):>4}"
    ...
    print(f"\n  IS champion: EMA{best_sp['ema_period']}"
          ...
          f" ethbtc={best_sp['ethbtc_short']}/{best_sp['ethbtc_long']}"
          f" mvrv<{best_sp['mvrv_threshold']}"
    ...
    champ_rpt.print_full_report(
        f"OOS CHAMPION — Combined Rotation"
        f"  ema={champ_sp['ema_period']}"
        f"  act={champ_sp['act_short']}/{champ_sp['act_long']}"
        f"  ethbtc={champ_sp['ethbtc_short']}/{champ_sp['ethbtc_long']}"
        f"  mvrv<{champ_sp['mvrv_threshold']}"
        f"  mhd={champ_sp.get('min_hold_days',0)}"
        f"  [STCG {TAX_RATE*100:.0f}%, {OOS_START}–{OOS_END}]"
    )

    # Walk-forward verdict on IS champion OOS
    if is_champ_oos:
        _, m_is, _ = is_champ_oos
    else:
        _, _, m_is, _ = oos_results[0]
        m_is = champ_m
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021–2024, Combined ETH/BTC Rotation + MVRV Gate")
    print("=" * 72)
