"""
Iteration 58: Filtered Donchian Breakout (BTC > 90d high + Price EMA + Vol Gate)
==================================================================================
Extends Iter 57 by adding BTC price > EMA as a supplementary entry filter.
This "filtered breakout" approach only enters when momentum AND trend both confirm.

Entry conditions (all must be true):
  1. BTC price > rolling N-day high (Donchian breakout momentum)
  2. BTC price > EMA(ema_period) (trend confirmation)
  3. BTC vol30 < vol_threshold (vol gate)

Exit conditions:
  BTC price < rolling M-day low (Donchian breakdown)
  — regime and vol do NOT trigger exits (preserves LTCG)

RATIONALE:
  Iter 57 (pure Donchian) failed MaxDD due to 2022 bear entries at breakout
  peaks that then reversed violently. Adding price > EMA as entry filter:
  1. Blocks 2021 peak entries (price breaks 90d high but EMA still bullish
     → enters; then if EMA flips bearish, no new entries allowed)
  2. Blocks 2022 false breakouts (BTC occasionally spikes above 90d high
     in bear markets, but price < EMA(100) blocks entry)
  3. Preserves 2020/2023/2024 genuine bull breakouts (both conditions true)

  IS 2018-2020 behavior expected:
    2018: BTC falling → no new 90d highs AND price < EMA → ZERO entries ✓
    2019 June surge: BTC breaks 90d high ($8-9k) AND price > EMA → entry?
          But June 2019 EMA(100) might still be < price → possible entry
    2020 July: BTC breaks $12k 90d high AND price > EMA → clean entry ✓

  Key win: IS champion selection should favor mhd=365 because:
    - 2018: zero false entries (both filters block)
    - 2019: minimal 1 entry if EMA confirms
    - mhd=365 IS hold from 2019 to IS end = massive winner
...
class FilteredDonchianStrategy:
    """
    Triple-condition entry: Donchian breakout + price EMA + vol gate.
    Exit: Donchian breakdown (rolling M-day low only).
    """
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        # 2. Donchian entry: new N-day high breakout
        roll_high    = btc_close.rolling(self.entry_lookback).max()
        donchian_in  = (btc_close > roll_high.shift(1)).fillna(False)

        # 3. Donchian exit: new M-day low breakdown
        roll_low     = btc_close.rolling(self.exit_lookback).min()
        donchian_out = (btc_close < roll_low.shift(1)).fillna(False)

        # 4. Vol gate (entry-only)
        btc_ret = btc_close.pct_change()
        vol30   = btc_ret.rolling(self.vol_lookback).std() * np.sqrt(252)
        vol_ok  = (vol30 < self.vol_threshold).fillna(False)

        # Combined entry: all three conditions
        entry_allowed = donchian_in & price_bull & vol_ok
        ...
        for date in all_dates:
            ...
            should_exit = bool(donchian_out.loc[date]) if date in donchian_out.index else False
...
def run_is(full_data, is_data, btc_full, eth_full, sp, bt_fixed):
    strat = FilteredDonchianStrategy(**sp)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 58 — Filtered Donchian (90d high + EMA + Vol Gate)")
    print("  Entry: BTC > N-day high AND price > EMA AND vol < threshold")
    print("  Exit:  BTC < M-day low (Donchian breakdown only)")
    ...
    strat_configs = [
        dict(ema_period=ema, entry_lookback=el, exit_lookback=xl,
    ...
    print(f"  {'ema':>4} {'entry':>6} {'exit':>5} {'vol_thr':>8} {'mhd':>4}"
    ...
    for sp in strat_configs:
        try:
            ...
            print(f"  {sp['ema_period']:>4} {sp['entry_lookback']:>6}  {sp['exit_lookback']:>4}"
    ...
    print(f"\n  IS champion: EMA{best_sp['ema_period']} entry={best_sp['entry_lookback']}d"
          f" exit={best_sp['exit_lookback']}d vol<{best_sp['vol_threshold']}"
    ...
    ic_rpt.print_full_report(
        f"IS-CHAMPION OOS — Filtered Donchian"
        f"  ema={ic_sp['ema_period']} entry={ic_sp['entry_lookback']}d"
        f"  exit={ic_sp['exit_lookback']}d vol<{ic_sp['vol_threshold']}"
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021–2024, Filtered Donchian")
    print("=" * 72)
