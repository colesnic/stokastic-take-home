"""
Iteration 21: BTC Trend × ETH Trend × BTC AdrActCnt (Dual Price + Network Adoption)
=====================================================================================
Innovation: Use ETH PRICE (not ETH on-chain) as the second regime filter.

WHY ETH price discriminates 2019:
  - 2019 was a BTC-only rally (+88%) while ETH barely moved ($130→$132, flat year)
  - ETH price was BELOW its 150d EMA for most of 2019 (EMA anchored to 2018 crash recovery)
  - This naturally blocks entries in 2019 without needing on-chain data for ETH

ADVANTAGES over previous strategies:
  - No dependence on L2 migration effects (ETH on-chain metrics decline due to L2s)
  - ETH price is clean, liquid data — no sampling/reporting lag
  - In 2023-2024, ETH recovered above its EMA → allows re-entry ✓
  - Uses BTC AdrActCnt (not TxCnt) for BTC network health — proven signal from Iter 15

Signal (triple confirmation):
  1. BTC price > EMA(ema_period)               [BTC macro trend]
  2. ETH price > EMA(ema_period)               [ETH ecosystem trend — NEW]
  3. BTC AdrActCnt_short > AdrActCnt_long      [BTC network adoption]

Enter: all THREE conditions bullish → hold BTC+ETH+BNB+ADA equally (25%)
...
def fetch_eth_price() -> pd.Series:
    """ETH close price — tries PriceUSD then ReferenceRateUSD."""
...
class DualTrendStrategy:
    """
    Triple confirmation using ETH PRICE (not ETH on-chain) as ecosystem signal:
      BTC price > EMA  AND  ETH price > EMA  AND  BTC AdrActCnt growing
    Universe: BTC+ETH+BNB+ADA (25% each)
    ETH price signal discriminates 2019 (ETH flat/below EMA while BTC rallied).
    """
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_price: pd.Series) -> None:
        # Signal 1: BTC price trend
        btc_close    = btc_full["Close"]
        btc_ema      = ema_fn(btc_close, self.ema_period)
        btc_bull     = btc_close > btc_ema

        # Signal 2: ETH price trend (reindexed to BTC dates)
        eth_ema      = ema_fn(eth_price, self.ema_period)
        eth_bull     = (eth_price > eth_ema).reindex(btc_close.index).ffill()

        # Signal 3: BTC active address momentum
        ...
        regime_bull  = btc_bull & eth_bull & btc_act_bull
...
def run_is(full_data, is_data, btc_full, eth_price, sp, bt_params):
    strat = DualTrendStrategy(**sp)
    strat.prepare(full_data, btc_full, eth_price)
    bt = AdvancedBacktester(initial_capital=100_000, **bt_params)
    return RiskReport(bt.run(is_data, strat))


def run_oos(full_data, combined_data, btc_full, eth_price, sp, bt_params) -> RiskReport | None:
    strat = DualTrendStrategy(**sp)
    strat.prepare(full_data, btc_full, eth_price)
    bt     = AdvancedBacktester(initial_capital=100_000, **bt_params)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 21 — BTC Trend × ETH Trend × BTC AdrActCnt")
    print("  ETH signal: PRICE (not on-chain) — blocks 2019 BTC-only rally")
    print(f"  Tax: STCG {TAX_RATE*100:.0f}%/<365d  |  LTCG 20%/≥365d")
    print("=" * 72)

    print("\n  Fetching BTC (price + AdrActCnt) and ETH (price only) ...")
    try:
        btc_full  = fetch_btc_full()
        eth_price = fetch_eth_price()
        print(f"  BTC: {len(btc_full)} rows  last=${float(btc_full['Close'].iloc[-1]):,.2f}")
        print(f"  ETH: {len(eth_price)} rows  last=${float(eth_price.iloc[-1]):,.2f}")
    ...
    # ETH price regime diagnostics
    ema150_eth = ema_fn(eth_price, 150)
    print(f"\n  ETH price vs 150d EMA regime by year:")
    print(f"  {'Year':<6}  {'%days above EMA':>16}  {'ETH start':>10}  {'EMA start':>10}")
    for yr in ["2018","2019","2020","2021","2022","2023","2024"]:
        mask = eth_price.index.year == int(yr)
        p = eth_price[mask].dropna()
        e = ema150_eth.reindex(p.index)
        if len(p) < 30:
            continue
        pct = float((p > e).mean()) * 100
        print(f"  {yr:<6}  {pct:>15.1f}%  {float(p.iloc[0]):>9.0f}   {float(e.iloc[0]):>9.0f}")
    ...
    for sp in strat_configs:
        try:
            rpt = run_is(full_data, is_data, btc_full, eth_price, sp, bt_fixed)
            m   = rpt.full_metrics()
    ...
    for _, _, _, _, _, sp, _ in results_is:
        try:
            rpt = run_oos(full_data, combined_data, btc_full, eth_price, sp, bt_fixed)
            if rpt is None:
                continue
            m   = rpt.full_metrics()
            hld = m.get("avg_holding_days", 0) or 0
    ...
    champ_rpt.print_full_report(
        f"OOS CHAMPION — Dual Trend + AdrActCnt  ema={champ_sp['ema_period']}"
    ...
    print(f"\n{'='*72}")
    print("  VERDICT  —  OOS 2021–2024, BTC × ETH Dual Trend × BTC AdrActCnt")
    print(f"{'='*72}")
    criteria = [
