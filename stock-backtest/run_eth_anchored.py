"""
Iteration 16: ETH-Anchored On-Chain — ETH as Regime Gate, No BTC
=================================================================
All previous passing iterations used BTC as the macro regime anchor.
This iteration tests whether ETH's own on-chain metrics can independently
time the alt-coin cycle — specifically BNB (Binance Smart Chain) and ADA
(Cardano), both of which are ETH ecosystem alternatives.

Signal (ETH-native, no BTC):
  1. ETH price > EMA(ema_period)               [ETH trend gate]
  2. ETH AdrActCnt_short > AdrActCnt_long      [ETH network adoption]

Enter: when BOTH conditions bullish → hold ETH + BNB + ADA equally (33.3%)
Exit: when EITHER fails (after min_hold_days)

Rationale: ETH leads the DeFi/smart contract ecosystem. BNB (Binance Smart Chain)
is an ETH-compatible fork. ADA competes for smart contract users that ETH also
serves. When ETH's own network is growing, demand for alternatives should follow.

This is a fundamentally different approach from BTC-anchored strategies:
  - BTC regime gate: macro monetary signal (institutional, store-of-value demand)
  - ETH regime gate: DeFi ecosystem signal (developer activity, dApp transactions)

Expected edges:
  2020 DeFi summer: ETH activity exploded → entry signal fires earlier than BTC
  2021: ETH activity grew throughout → captures full alt bull run
  2022: ETH activity fell → exits before or during crash
  2023-2024: ETH L2 migration slows L1 addresses, might cause missed entries
...
class EthAnchoredStrategy:
    """
    ETH-anchored on-chain regime. No BTC involvement.
    ETH price > EMA AND ETH active addresses growing → hold ETH+BNB+ADA equally.
    ...
    def prepare(self, data: dict, eth_full: pd.DataFrame) -> None:
        ...
        all_dates = eth_close.index.intersection(
...
def run_is(full_data: dict, is_data: dict, eth_full: pd.DataFrame,
           sp: dict, bt_params: dict) -> RiskReport:
    strat = EthAnchoredStrategy(**sp)
    strat.prepare(full_data, eth_full)
    bt = AdvancedBacktester(initial_capital=100_000, **bt_params)
    return RiskReport(bt.run(is_data, strat))


def run_oos(full_data: dict, combined_data: dict, eth_full: pd.DataFrame,
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 16 — ETH-Anchored On-Chain: ETH+BNB+ADA")
    print("  ETH regime gate only — no BTC signal")
    print(f"  Tax: STCG {TAX_RATE*100:.0f}%/<365d  |  LTCG 20%/≥365d")
    print("=" * 72)

    print("\n  Fetching ETH (with on-chain), BNB, ADA ...")
    try:
    ...
    # ETH active address statistics
    eth_adr = eth_full["AdrActCnt"].ffill()
    print(f"\n  ETH active address growth (regime signal):")
    print(f"  {'Year':<10}  {'Avg AdrAct':>11}  {'Start':>11}  {'End':>11}  {'Change%':>9}")
    for yr in ["2018","2019","2020","2021","2022","2023","2024"]:
        s = eth_adr[eth_adr.index.year == int(yr)]
        if len(s) > 1:
            chg = (float(s.iloc[-1])/float(s.iloc[0])-1)*100
            print(f"  {yr:<10}  {s.mean():>11,.0f}  {float(s.iloc[0]):>11,.0f}  "
                  f"{float(s.iloc[-1]):>11,.0f}  {chg:>+8.1f}%")
    ...
    for sp in strat_configs:
        try:
            rpt = run_is(full_data, is_data, eth_full, sp, bt_fixed)
            m   = rpt.full_metrics()
    ...
    for _, _, _, _, _, sp, _ in results_is:
        try:
            rpt = run_oos(full_data, combined_data, eth_full, sp, bt_fixed)
            if rpt is None:
                continue
            m   = rpt.full_metrics()
            hld = m.get("avg_holding_days", 0) or 0
    ...
    champ_rpt.print_full_report(
        f"OOS CHAMPION — ETH+BNB+ADA  ema={champ_sp['ema_period']}"
    ...
    print(f"\n{'='*72}")
    print("  VERDICT  —  OOS 2021–2024, ETH-Anchored Strategy")
    print(f"{'='*72}")
    criteria = [
