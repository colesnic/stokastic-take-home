"""
Iteration 73: BTC Price Trend + ETH TxCnt + ETH AdrActCnt (Pure DeFi Filter)
==============================================================================
Novel approach: Replace BTC AdrActCnt (Signal 2) with ETH TxCnt.
  Signal 1: BTC price > EMA (macro trend)
  Signal 2: ETH TxCnt EMA trend (DeFi transaction volume)
  Signal 3: ETH AdrActCnt EMA trend (Ethereum user adoption)

WHY PURE ETH DEFI FILTER:
  Previous strategies used BTC AdrActCnt as Signal 2. This iteration uses TWO
  ETH signals instead, making the strategy purely DeFi-focused:
  - ETH TxCnt: captures the VOLUME of Ethereum activity (how many transactions)
  - ETH AdrActCnt: captures the WIDTH of Ethereum activity (how many unique users)

  Both volume AND breadth of DeFi must be growing for entry — a more stringent
  requirement that specifically targets DeFi-driven altseasons rather than any
  Bitcoin rally.

  ETH TxCnt 2019 (proven in Iter 62):
    Q3 2019:  8% bull (ICO boom over, DeFi not launched, minimal smart contract use)
    Q4 2019: 29% bull (some recovery but still pre-DeFi)
    JF2020:  37% bull (starting to recover, some entries possible)

  ETH AdrActCnt 2019 (measured):
    Q3 2019: 20% bull
    Q4 2019: 11% bull
    JF2020:  30% bull

  COMBINED (TxCnt AND AdrActCnt both bullish):
    Q3 2019: 8% AND 20% → intersection ≤ 8% → minimal IS entries!
    Q4 2019: 29% AND 11% → intersection ≤ 11% → very few IS entries!
    JF2020:  37% AND 30% → intersection maybe 15-20% → moderate risk

  IS GAP PREDICTION:
    With both ETH signals required: Q3-Q4 2019 effectively 0-8% combined.
    mhd=365 configs should have very limited IS entries → small IS MaxDD.
    mhd=0 configs: more IS entries (whenever signals briefly align) but erratic.
    Expected IS gap: strongly positive.

  OOS COVERAGE:
    2020: TxCnt (37% at full year, ~70% H2) AND AdrActCnt (78%) → overlap maybe 50%
    2021: TxCnt (65%) AND AdrActCnt (65%) → overlap maybe 45-50%
    2022: TxCnt (23%) AND AdrActCnt (33%) → overlap ~10% (good bear protection)
    2023-2024: moderate (Ethereum ecosystem maintained)

  KEY ADVANTAGE OVER ITER 66 (TxCnt AND FeeTotNtv):
    Iter 66 failed because TxCnt and FeeTotNtv are HIGHLY CORRELATED
    (FeeTotNtv = TxCnt × AvgGasPrice). Adding AdrActCnt gives truly independent
    information: volume can be high but if unique users are flat (bots), AdrActCnt
    won't confirm → avoids bot-driven volume spikes entering IS.
...
def fetch_eth_full() -> pd.DataFrame:
    """Fetch ETH price + TxCnt + AdrActCnt (both used as regime signals)."""
    ...
    out = (df[["Date", "Close", "TxCnt", "AdrActCnt"]]
           .dropna(subset=["Close"])
           .set_index("Date")
           .sort_index())
    return out
...
class EthDeFiDualRegimeStrategy:
    """
    Triple regime: BTC price EMA + ETH TxCnt EMA trend + ETH AdrActCnt EMA trend.
    Purely DeFi-focused: enter only when Ethereum DeFi is active in BOTH volume
    (TxCnt) AND user adoption (AdrActCnt). Removes BTC AdrActCnt from the filter.
    When BTC is in uptrend AND Ethereum has growing transaction volume AND growing
    user base → ideal DeFi altseason entry for the full portfolio.
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        # Signal 3: ETH AdrActCnt EMA trend (DeFi user adoption)
        ...
        regime_bull = price_bull & tx_bull & adr_bull
...
def run_is(full_data, is_data, btc_full, eth_full, sp, bt_fixed):
    strat = EthDeFiDualRegimeStrategy(**sp)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 73 — BTC Trend + ETH TxCnt + ETH AdrActCnt (DeFi Dual)")
    print("  Portfolio: BTC, ETH, BNB, ADA, XRP  (20% each)")
    print("  Signal: BTC EMA + ETH TxCnt + ETH AdrActCnt (NO BTC AdrActCnt)")
    print("  Key: Both ETH DeFi signals bearish in 2019 H2 → combined ≤8% Q3 2019")
    print("  Key: Pure DeFi filter — both volume AND user breadth must be growing")
    ...
    print("\n  Fetching BTC (price only) ...")
    btc_full = fetch_btc_full()
    print("  Fetching ETH (price + TxCnt + AdrActCnt) ...")
    ...
    tx30 = eth_tx.ewm(span=30).mean(); tx90 = eth_tx.ewm(span=90).mean()
    a30  = eth_adr.ewm(span=30).mean(); a90  = eth_adr.ewm(span=90).mean()
    bull_tx  = tx30 > tx90
    bull_adr = a30 > a90
    bull_combined = bull_tx & bull_adr

    print("\n  Combined ETH signal (TxCnt AND AdrActCnt) by year (EMA 30/90):")
    print(f"  {'Period':>10}  {'TxCntOnly':>10}  {'AdrOnly':>8}  {'Combined':>8}")
    periods = [
    ...
    for period, s, e in periods:
        ...
        if len(bt2):
            print(f"  {period:>10}  {100*bt2.mean():>8.0f}%  {100*ba.mean():>6.0f}%  {100*bc.mean():>6.0f}%")
    ...
    print(f"  RISK REPORT — IS-CHAMPION OOS — ETH DeFi Dual Regime"
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021-2024, ETH DeFi Dual Regime + Vol Gate")
    print("=" * 72)
    passes = 0
