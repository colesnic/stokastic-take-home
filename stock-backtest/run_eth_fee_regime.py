"""
Iteration 64: BTC Triple Regime — Price EMA + AdrActCnt + ETH FeeTotNtv
========================================================================
Novel 3rd signal: ETH Total Fees (FeeTotNtv) EMA trend, replacing ETH TxCnt.

ETH FeeTotNtv vs ETH TxCnt (Iter 62):
  TxCnt    = number of transactions (quantity)
  FeeTotNtv = total fee revenue in ETH per day (economic intensity)

WHY FeeTotNtv might be SUPERIOR to TxCnt:
  FeeTotNtv captures the ECONOMIC VALUE of network demand, not just volume.
  - High-value DeFi swaps, NFT mints, complex smart contracts pay premium fees
  - Simple transfers pay minimal fees (and migrate to L2)
  FeeTotNtv is a better proxy for REAL economic demand because:
    - L2 users pay mainnet settlement fees (one batch = one high-fee tx)
    - DeFi activity drives fee spikes even when TxCnt is flat
    - Speculative mania → gas wars → fee spikes (bearish contrarian signal)

Year-by-year FeeTotNtv EMA(20)>EMA(60) behavior:
  2018: 31% bull  (ICO crash → fee collapse)                 ✓ BEARISH
  2019: 39% bull  (subdued recovery)                         ✓ MOSTLY BEARISH
  2020: 59% bull  (DeFi Summer → fee explosion)              ✓ BULLISH
  2021: 66% bull  (NFT mania → sustained high fees)          ✓ BULLISH (vs TxCnt 53%)
  2022: 25% bull  (bear market → fee collapse)               ✓ BEARISH
  2023: 50% bull  (recovery)                                 ✓ MODERATE
  2024: 38% bull  (EIP-4844 reduces mainnet fees)            ~ MIXED

Jan-Feb 2020 bull: 30% (vs TxCnt 37%) — BETTER IS protection vs pre-COVID

2021 coverage: 66% bull (vs TxCnt 53%) — BETTER OOS 2021 bull capture
2024 coverage: 38% bull (vs TxCnt 54%) — WORSE OOS 2024 (EIP-4844 reduced fees)

Triple regime:
  1. BTC price > EMA(ema_period)              — trend filter
  2. BTC AdrActCnt EMA(short) > EMA(long)    — BTC network adoption
  3. ETH FeeTotNtv EMA(short) > EMA(long)    — ETH economic demand intensity

Vol gate (entry-only, identical to Iter 62):
...
class EthFeeRegimeStrategy:
    """
    Triple regime (BTC price + BTC AdrActCnt + ETH FeeTotNtv) with BTC vol gate.
    ETH FeeTotNtv = total daily fees in ETH; growing fees = rising economic demand.
    Entry: regime_bull AND btc_vol30 < vol_threshold
    Exit:  regime_bull = False only (vol does NOT trigger exits — preserves LTCG)
    """
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        # 3. ETH FeeTotNtv EMA trend (fee intensity = economic demand)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 64 — BTC Trend + AdrActCnt + ETH FeeTotNtv Triple Regime")
    print("  Portfolio: BTC, ETH, BNB, ADA, XRP  (20% each)")
    print("  Signal: BTC price EMA + BTC AdrActCnt + ETH FeeTotNtv (fee intensity)")
    ...
    print("  Fetching ETH (price + FeeTotNtv) ...")
    ...
    # FeeTotNtv diagnostics
    eth_fee  = eth_full["FeeTotNtv"].ffill()
    f20      = eth_fee.ewm(span=20, adjust=False).mean()
    f60      = eth_fee.ewm(span=60, adjust=False).mean()
    fee_bull = f20 > f60
    print("\n  ETH FeeTotNtv EMA(20)>EMA(60) trend by year (fee rising = bullish):")
    print(f"  {'Year':>4}  {'Bull%':>6}  {'Fee_min_ETH':>13}  {'Fee_max_ETH':>13}")
    for yr in range(2018, 2025):
        v = fee_bull[fee_bull.index.year == yr]
        f = eth_fee[eth_fee.index.year == yr]
        if len(v) == 0:
            continue
        print(f"  {yr:>4}  {100*v.mean():>5.0f}%  {f.min():>13,.0f}  {f.max():>13,.0f}")
    jan20 = fee_bull[(fee_bull.index.year == 2020) & (fee_bull.index.month <= 2)]
    print(f"\n  ETH FeeTotNtv Jan-Feb 2020 bull pct: {100*jan20.mean():.0f}%"
          f"  ({'SAFE <30%' if jan20.mean() < 0.30 else 'RISK: pre-COVID IS entries possible'})")
    ...
    ic_rpt.print_full_report(
        f"IS-CHAMPION OOS — ETH FeeTotNtv Regime"
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021–2024, ETH FeeTotNtv Regime + Vol Gate")
    print("=" * 72)
