"""
Iteration 68: BTC Trend + AdrActCnt + ETH Avg Gas Price Regime
================================================================
Novel 3rd signal: ETH FeeTotNtv / ETH TxCnt = average fee per transaction.
This is the "Ethereum Gas Price Index" — the economic cost of each ETH
transaction, capturing network congestion and DeFi economic intensity.

WHY ETH AVG GAS PRICE (FeeTotNtv/TxCnt):
  ETH TxCnt alone (Iter 62): captures activity volume, bearish Q3 2019 (8%).
  ETH FeeTotNtv alone (Iter 64): captures total fees, spiky/volatile, IS gap +2.12.
  FeeTotNtv/TxCnt = avg fee per tx: captures the PRICE of ETH block space.

  KEY PROPERTY: Average gas price was VERY LOW in 2019 (pre-DeFi era):
    - ICO boom over → minimal smart contract activity → low gas
    - DeFi hadn't launched → gas was near base fee level
    - Only simple transfers → low competition for block space

  QUARTERLY 2019 BULL% (EMA 30/90):
    Q1: 34%  Q2: 0%  Q3: 40%  Q4: 18%  Full Year: 23%
    → Very few IS entries allowed (only 23% of 2019 days are bullish)
    → Q4 2019 (18% bull) + BTC likely below EMA100 during BTC decline → near-zero entry window

  JAN-FEB 2020: 7% bull (EXCELLENT IS protection — almost no pre-COVID entries)

  YEAR-BY-YEAR OOS COVERAGE (EMA 30/90):
    2021 = 63% (Q1=100% DeFi fever, Q2=20% crash, Q3=53% recovery, Q4=78% NFT)
    2022 = 23% (good bear protection — gas prices collapsed in bear market)
    2023 = ~38% (recovery phase)
    2024 = 39% (post-EIP-4844: blob fees separate from execution fees, mainnet
              gas prices actually ROSE for execution txs in 2024 bull run)

  IS GAP PREDICTION: Large positive gap expected because:
    1. mhd=0 strategies get punished by gas price spikes: high gas = congestion
       = late-party entry = local price top → rapid exits = many small losses
    2. mhd=365 strategies ignore gas spikes (30/90 EMA is too slow to respond
       to individual spike events) → trades on sustained demand trends only
    3. 2019 full year only 23% bull → few IS entries → IS MaxDD stays manageable

  WHY DIFFERENT FROM ETH FeeTotNtv (Iter 64):
    FeeTotNtv = TxCnt × AvgGasPrice → conflates volume and price effects
    AvgGasPrice = FeeTotNtv / TxCnt → PURE price signal (isolates congestion)

    During L2 adoption: TxCnt may fall (L2 takes tx volume) but AvgGasPrice
    can RISE if remaining mainnet txs are high-value DeFi (MEV, large swaps)
    This makes AvgGasPrice a purer signal of "economic intensity per mainnet tx"
...
def fetch_eth_full() -> pd.DataFrame:
    """Fetch ETH price + FeeTotNtv + TxCnt to compute avg gas price."""
    ...
    out = df[["Date", "Close", "FeeTotNtv", "TxCnt"]].dropna(subset=["Close"]).set_index("Date").sort_index()
    # Compute avg gas price (fee per tx in ETH)
...
class EthGasPriceRegimeStrategy:
    """
    Triple regime: BTC price EMA + BTC AdrActCnt trend + ETH AvgGasPrice trend.
    ETH AvgGasPrice = FeeTotNtv / TxCnt = average fee per transaction (in ETH).
    This isolates the PRICE dimension of Ethereum block space demand:
      - Rising gas price → high economic competition for block space → DeFi/NFT bullish
      - Falling gas price → low demand for block space → bearish
    Signal was very bearish in 2019 (pre-DeFi) and bullish during DeFi/NFT eras.
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        # Signal 3: ETH average gas price EMA trend (block space congestion)
...
def run_is(full_data, is_data, btc_full, eth_full, sp, bt_fixed):
    strat = EthGasPriceRegimeStrategy(**sp)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 68 — BTC Trend + AdrActCnt + ETH Avg Gas Price Regime")
    print("  Portfolio: BTC, ETH, BNB, ADA, XRP  (20% each)")
    print("  Signal: BTC EMA + BTC AdrActCnt + ETH AvgGasPrice (FeeTotNtv/TxCnt)")
    print("  Key: Gas price VERY LOW in 2019 pre-DeFi (23% full year) → IS protection")
    print("  Key: JF2020=7% (near-perfect), 2021=63%, 2022=23% (good bear)")
    ...
    print("  Fetching ETH (price + FeeTotNtv + TxCnt for gas price) ...")
    ...
    # Gas price diagnostics
    gas = eth_full["AvgGasPrice"].ffill()
    print("\n  ETH AvgGasPrice EMA trend by year (EMA 30/90):")
    g30 = gas.ewm(span=30).mean(); g90 = gas.ewm(span=90).mean()
    bull30 = g30 > g90
    print(f"  {'Year':>4}  {'Bull%':>8}  {'AvgGas_min':>14}  {'AvgGas_max':>14}")
    for yr in range(2018, 2025):
        v = bull30[bull30.index.year == yr]
        t = gas[gas.index.year == yr]
        if not len(v): continue
        print(f"  {yr:>4}  {100*v.mean():>6.0f}%  {t.min():>14.8f}  {t.max():>14.8f}")
    jf = bull30[(bull30.index >= "2020-01-01") & (bull30.index < "2020-03-01")]
    print(f"\n  Jan-Feb 2020 gas price bull%: {100*jf.mean():.0f}%", end="")
    if jf.mean() <= 0.10:
        print("  → GREAT: minimal pre-COVID IS entries!")
    elif jf.mean() <= 0.25:
        print("  → GOOD")
    else:
        print()
    ...
    print(f"  RISK REPORT — IS-CHAMPION OOS — ETH Gas Price Regime"
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021-2024, ETH Gas Price Regime + Vol Gate")
    print("=" * 72)
    passes = 0
