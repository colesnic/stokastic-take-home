"""
Iteration 59: BTC Realized Volatility Entry Gate — 5-Coin Portfolio — ETH MVRV as 3rd Regime
================================================================================
Extends Iter 27 (Vol Gate, 7/7 PASS at +2.94%/mo) by adding TRX as a 5th coin.
TRX returned +844% in OOS 2021-2024 vs ADA's +380%.

Novel signal: BTC 30-day realized volatility as ENTRY-ONLY gate.

RATIONALE (identical to Iter 27):
  High realized vol = explosive/unstable price movement → bad entry timing
  Low realized vol = trend compression → better risk/reward entry

  BTC vol by period:
    Jan 2018 (ATH): 126% annualized → BLOCKS entry at bubble peak
    Jan 2021 (OOS): 51% annualized  → ALLOWS entry at bull run start
    Mar 2020 (COVID): 127%          → entry gate active (but won't force exits)
    Jun 2021 (correction): 85%      → <100% threshold allows, <80% blocks

WHY TRX (identical reasoning to Iter 40):
  TRX IS 2020: +104% (moderate, won't skew IS champion toward mhd=0)
  TRX OOS 2021-2024: +844% (far outperforms ADA at +380%)
  TRX available from Jun 2018 in Coinmetrics → full IS-relevant history
  MVRV/regime correctly times TRX entries alongside other coins
...
Triple regime (same as Iter 20/27):
  1. BTC price > EMA(ema_period)
  2. BTC AdrActCnt EMA(short) > EMA(long)
  3. ETH MVRV EMA(short) > EMA(long)
...
class ETHMVRVRegimeStrategy:
    """
    Triple signal regime (BTC price + AdrActCnt + ETH TxCnt) with
    BTC realized vol < threshold as an ENTRY-ONLY gate. 5-coin portfolio.
    Entry: regime_bull AND btc_vol30 < vol_threshold
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        eth_mvrv   = eth_full["MVRV"].ffill()
        mv_s       = ema_fn(eth_mvrv, self.act_short)
        mv_l       = ema_fn(eth_mvrv, self.act_long)
        mv_bull    = (mv_s > mv_l).reindex(btc_close.index).ffill().fillna(False)

        regime_bull = price_bull & adr_bull & mv_bull
...
def run_is(full_data, is_data, btc_full, eth_full, sp, bt_fixed):
    strat = ETHMVRVRegimeStrategy(**sp)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 59 — BTC Realized Vol Gate + 5-Coin Portfolio — ETH MVRV as 3rd Regime")
    ...
    print("\n  BTC 30d realized vol by year:")
    print(f"  {'Year':>4}  {'Min':>8}  {'Max':>8}  {'Jan 1':>8}  {'Below 60%':>10}  {'Below 80%':>10}")
    for yr in range(2017, 2025):
        v = vol30[vol30.index.year == yr]
        if len(v) == 0:
            continue
        jan1 = v.iloc[0]
        pct60 = (v < 0.60).mean() * 100
        pct80 = (v < 0.80).mean() * 100
        print(f"  {yr:>4}  {v.min():>8.2f}  {v.max():>8.2f}  {jan1:>8.2f}  {pct60:>9.1f}%  {pct80:>9.1f}%")
    ...
    champ_rpt.print_full_report(
        f"OOS CHAMPION — ETH MVRV Regime"
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021–2024, BTC Vol Gate + 5-Coin Portfolio")
    print("=" * 72)
