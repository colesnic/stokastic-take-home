"""
Iteration 60: BTC Triple Regime — Price EMA + AdrActCnt + ETH TxTfrCnt
=======================================================================
Novel 3rd signal: ETH Transfer Count (TxTfrCnt) EMA trend.

ETH TxTfrCnt vs ETH TxCnt (Iter 41):
  TxCnt    = ALL transactions including coinbase (miner/validator rewards)
  TxTfrCnt = Transfer-only transactions (excludes miner/validator coinbase)

WHY TxTfrCnt IS SUPERIOR to TxCnt as a regime signal:
  TxTfrCnt is a cleaner measure of REAL USER ACTIVITY on the ETH network:
  - Excludes validator reward transactions (post-Merge), which are protocol
    mechanics, not market activity signals
  - More directly tracks genuine network demand (DeFi, NFTs, user transfers)
  - Result: stronger bearish signal in IS 2018 (25% bull vs TxCnt's 34%)
    → potentially wider IS champion selection gap vs mhd=0

Year-by-year ETH TxTfrCnt EMA trend behavior:
  2018: 25% bull  (BTC crash → ETH activity collapse)            ✓ BEARISH
  2019: 36% bull  (recovery phase, still subdued)                ✓ NEUTRAL
  2020: 70% bull  (DeFi summer → genuine user activity surge)    ✓ BULLISH
  2021: 73% bull  (DeFi/NFT mania → high genuine transfers)      ✓ BULLISH
  2022: 42% bull  (bear market → declining real activity)        ✓ BEARISH
  2023: 65% bull  (recovery + L2 batch rollup transactions)      ✓ BULLISH
  2024: 63% bull  (continued activity despite ETH underperf)     ✓ BULLISH

NOTE: L2 rollup settlements on ETH mainnet INCREASE TxTfrCnt — each L2
batch appears as a mainnet transfer. Unlike ETH TxCnt which declined from
L2 migration (Iter 41 risk), TxTfrCnt may be more L2-resilient.

Triple regime (BTC price + BTC AdrActCnt + ETH TxTfrCnt):
  1. BTC price > EMA(ema_period)              — trend filter (MaxDD protector)
  2. BTC AdrActCnt EMA(short) > EMA(long)    — BTC network adoption
  3. ETH TxTfrCnt EMA(short) > EMA(long)     — ETH genuine user activity
...
class EthTxTfrCntRegimeStrategy:
    """
    Triple regime (BTC price + BTC AdrActCnt + ETH TxTfrCnt) with BTC vol gate.
    Entry: regime_bull AND btc_vol30 < vol_threshold
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        # 3. ETH TxTfrCnt EMA trend (genuine user transfer activity)
        eth_tx     = eth_full["TxTfrCnt"].ffill()
...
def run_is(full_data, is_data, btc_full, eth_full, sp, bt_fixed):
    strat = EthTxTfrCntRegimeStrategy(**sp)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 60 — BTC Price + AdrActCnt + ETH TxTfrCnt Triple Regime")
    ...
    # ETH TxTfrCnt diagnostics
    eth_tx   = eth_full["TxTfrCnt"].ffill()
    tx_s20   = eth_tx.ewm(span=20, adjust=False).mean()
    tx_l60   = eth_tx.ewm(span=60, adjust=False).mean()
    tx_bull  = tx_s20 > tx_l60
    print("\n  ETH TxTfrCnt EMA(20)>EMA(60) trend by year:")
    print(f"  {'Year':>4}  {'Bull%':>6}  {'TxTfrCnt_min':>13}  {'TxTfrCnt_max':>13}  {'mean':>10}")
    for yr in range(2017, 2025):
        v = tx_bull[tx_bull.index.year == yr]
        t = eth_tx[eth_tx.index.year == yr]
        if len(v) == 0:
            continue
        print(f"  {yr:>4}  {100*v.mean():>5.0f}%"
              f"  {t.min():>13,.0f}  {t.max():>13,.0f}  {t.mean():>10,.0f}")
    ...
    ic_rpt.print_full_report(
        f"IS-CHAMPION OOS — ETH TxTfrCnt Regime"
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021–2024, ETH TxTfrCnt Regime + Vol Gate")
    print("=" * 72)
