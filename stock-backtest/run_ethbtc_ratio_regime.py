"""
Iteration 69: BTC Trend + AdrActCnt + ETH/BTC Ratio (Altseason Indicator)
==========================================================================
Novel 3rd signal: ETH/BTC price ratio EMA trend = "altseason" indicator.

WHY ETH/BTC RATIO:
  When ETH outperforms BTC (ETH/BTC rising), capital is rotating from BTC
  into altcoins — this is "altseason." Rising ETH/BTC signals:
    - DeFi activity driving ETH demand
    - Risk appetite expanding to altcoins (good for BNB, ADA, XRP)
    - Institutional and retail funds flowing into the broader crypto market

  When ETH/BTC is falling, BTC dominance is rising — capital is rotating
  INTO BTC from alts, or leaving crypto entirely. This is bearish for the
  portfolio of alts (ETH, BNB, ADA, XRP) and even for BTC itself late-cycle.

KEY PROPERTY — 2019 downtrend (IS protection):
  Jan 2019:   BTC $3.5k, ETH $150 → ETH/BTC ratio ~0.043
  Jun 2019:   BTC $12k,  ETH $300 → ETH/BTC ratio ~0.025 (BTC ran 3.4x, ETH 2x)
  Sep 2019:   BTC $10k,  ETH $180 → ETH/BTC ratio ~0.018 (both fell, ETH more)
  Dec 2019:   BTC $7.2k, ETH $130 → ETH/BTC ratio ~0.018 (still near bottom)
  Feb 2020:   BTC $9.5k, ETH $235 → ETH/BTC ratio ~0.025 (slight recovery)

  EMA(30) vs EMA(90) on ETH/BTC ratio:
    - Q1 2019: ratio declining → short < long → bearish signal
    - Q2 2019: BTC surges harder → ratio falls further → still bearish
    - Q3-Q4 2019: ratio at multi-year lows → strongly bearish → NO IS ENTRIES
    - Jan-Feb 2020: just starting to recover → still bearish → NO PRE-COVID IS ENTRIES

  HYPOTHESIS: ETH/BTC ratio had ~0-15% bull days in Q3-Q4 2019
  (ratio was near the bottom of its 2019-2020 range, EMA short well below long)
  → Near-zero IS entry window → IS protection even better than AvgGasPrice

KEY PROPERTY — OOS coverage:
  DeFi Summer 2020 (Jun-Dec): ETH/BTC surged from 0.02 to 0.032 → bullish
  2021 Q1-Q2: ETH/BTC hit 0.06-0.08 (altseason peak) → very bullish
  2021 Q2: BTC/ETH correction → mixed
  2021 Q4: ETH/BTC at 0.065-0.075 with NFT mania → bullish
  2022: ETH/BTC declined (bear market, BTC dominance recovering) → exits
  2023: Recovery, ETH/BTC stabilized → moderate coverage
  2024: BTC ETF narrative → BTC outperformed → ETH/BTC fell → bearish (selective)

  Selectivity in 2024 is appropriate: BTC ETF-driven rally didn't benefit alts
  proportionally. A bearish ETH/BTC signal in 2024 prevents buying ADA/XRP
  at peak BTC dominance.

EXPECTED IS GAP:
  ETH/BTC ratio was at multi-year lows in Q3-Q4 2019 → mhd=0 can't profitably
  trade (little bull signal), mhd=365 avoids IS entry trap entirely → large IS gap
...
def fetch_eth_full() -> pd.DataFrame:
    """Fetch ETH price — we only need Close for ETH/BTC ratio."""
...
class EthBtcRatioRegimeStrategy:
    """
    Triple regime: BTC price EMA + BTC AdrActCnt trend + ETH/BTC ratio trend.
    ETH/BTC ratio = altseason indicator.
      Rising ETH/BTC: capital rotating into alts (DeFi, NFTs) → broad crypto bullish.
      Falling ETH/BTC: BTC dominance rising (risk-off or BTC-specific rally) → avoid alts.
    Signal was bearish through 2019 (BTC dominance era) and bullish during 2020-2021 altseason.
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        # Signal 3: ETH/BTC price ratio EMA trend (altseason indicator)
        eth_close = eth_full["Close"].reindex(btc_close.index).ffill()
        eth_btc_ratio = eth_close / btc_close  # e.g. 0.02-0.08 range
        ...
        regime_bull = price_bull & adr_bull & ratio_bull
...
def run_is(full_data, is_data, btc_full, eth_full, sp, bt_fixed):
    strat = EthBtcRatioRegimeStrategy(**sp)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 69 — BTC Trend + AdrActCnt + ETH/BTC Ratio (Altseason)")
    print("  Portfolio: BTC, ETH, BNB, ADA, XRP  (20% each)")
    print("  Signal: BTC EMA + BTC AdrActCnt + ETH/BTC ratio EMA trend")
    print("  Key: ETH/BTC in downtrend 2019 (BTC dominance era) → IS protection")
    print("  Key: ETH/BTC bullish in 2020-2021 DeFi/altseason → OOS coverage")
    ...
    print("  Fetching ETH (price for ETH/BTC ratio) ...")
    ...
    eth_close_diag = eth_full["Close"].reindex(btc_full.index).ffill()
    ratio = eth_close_diag / btc_full["Close"]
    r30 = ratio.ewm(span=30).mean()
    r90 = ratio.ewm(span=90).mean()
    bull_ratio = r30 > r90

    print("\n  ETH/BTC ratio EMA trend by year (EMA 30/90):")
    print(f"  {'Year':>4}  {'Bull%':>8}  {'Ratio_min':>12}  {'Ratio_max':>12}  {'Ratio_mean':>12}")
    for yr in range(2018, 2025):
        v = bull_ratio[bull_ratio.index.year == yr]
        t = ratio[ratio.index.year == yr]
        if not len(v): continue
        print(f"  {yr:>4}  {100*v.mean():>6.0f}%  {t.min():>12.4f}  {t.max():>12.4f}  {t.mean():>12.4f}")

    q = {}
    for yr, q_name, qstart, qend in [
        (2019, "Q3-2019", "2019-07-01", "2019-09-30"),
        (2019, "Q4-2019", "2019-10-01", "2019-12-31"),
        (2020, "JF-2020", "2020-01-01", "2020-02-29"),
    ]:
        v = bull_ratio[(bull_ratio.index >= qstart) & (bull_ratio.index <= qend)]
        q[q_name] = 100 * v.mean() if len(v) else 0
        print(f"  {q_name}: {q[q_name]:.0f}% bull", end="")
        if q_name == "JF-2020":
            if q[q_name] <= 10:
                print("  → EXCELLENT pre-COVID IS protection!")
            elif q[q_name] <= 25:
                print("  → GOOD")
            else:
                print("  → WARNING: possible pre-COVID IS entries")
        else:
            if q[q_name] <= 20:
                print("  → EXCELLENT 2019 H2 bearishness!")
            elif q[q_name] <= 35:
                print("  → GOOD")
            else:
                print("  → WARNING: 2019 H2 may cause IS entry trap")
    ...
    print(f"  RISK REPORT — IS-CHAMPION OOS — ETH/BTC Ratio Regime"
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021-2024, ETH/BTC Ratio Regime + Vol Gate")
    print("=" * 72)
    passes = 0
