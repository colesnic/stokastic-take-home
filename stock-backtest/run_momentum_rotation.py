"""
Iteration 46: Cross-Sectional Momentum Rotation
=================================================
Strategy: Each month (30-day rebalance), hold the top-2 coins by trailing
N-day price return. Regime gate: BTC price > EMA(period) — bearish regime
exits all positions.

Universe: BTC, ETH, BNB, ADA, TRX
Portfolio: Top-2 coins, 50% / 50%

Rationale:
  Cross-sectional momentum (relative strength rotation) is one of the most
  documented factors in finance (Jegadeesh & Titman 1993, AQR crypto studies).
  In crypto, the "hot coin" rotates: ETH 2020-21, BNB 2021, BTC 2023 (Ordinals
  + ETF), various alts 2024. Rotating into momentum leaders and out of laggards
  should capture this dispersion while the BTC regime gate avoids entering
  during broad bear markets.

Walk-forward: IS 2018-2020, OOS 2021-2024
Tax: 35% STCG <365d  |  20% LTCG >=365d
"""

import urllib.request
import io
import os
import sys
import numpy as np
import pandas as pd
import warnings
warnings.filterwarnings("ignore")

sys.path.insert(0, os.path.dirname(__file__))

from backtester_advanced import AdvancedBacktester, AdvancedBacktestResult
from risk_metrics import RiskReport
from indicators import ema as ema_fn, atr as atr_fn, adx as adx_fn

COINMETRICS = "https://raw.githubusercontent.com/coinmetrics/data/master/csv/{}.csv"

IS_START  = "2018-01-01"
IS_END    = "2020-12-31"
OOS_START = "2021-01-01"
OOS_END   = "2024-12-31"
TAX_RATE  = 0.35

COINS = [
    ("btc", "BTC", 42),
    ("eth", "ETH", 11),
    ("bnb", "BNB", 77),
    ("ada", "ADA", 99),
    ("trx", "TRX", 55),
]

N_TOP = 2  # hold top-2 coins, 50% each


def fetch_btc_full() -> pd.DataFrame:
    url = COINMETRICS.format("btc")
    with urllib.request.urlopen(url, timeout=20) as r:
        df = pd.read_csv(io.StringIO(r.read().decode()), low_memory=False)
    df["Date"]  = pd.to_datetime(df["time"])
    df["Close"] = pd.to_numeric(df["PriceUSD"], errors="coerce")
    return df[["Date", "Close"]].dropna().set_index("Date").sort_index()


def fetch_close(coin: str) -> pd.Series:
    url = COINMETRICS.format(coin)
    with urllib.request.urlopen(url, timeout=20) as r:
        df = pd.read_csv(io.StringIO(r.read().decode()), low_memory=False)
    df["Date"] = pd.to_datetime(df["time"])
    for col in ("PriceUSD", "ReferenceRateUSD"):
        if col in df.columns:
            df["Close"] = pd.to_numeric(df[col], errors="coerce")
            if df["Close"].notna().sum() > 100:
                break
    return df[["Date", "Close"]].dropna().set_index("Date").sort_index()["Close"]


def synthesize_ohlcv(close: pd.Series, seed: int = 42) -> pd.DataFrame:
    rng  = np.random.default_rng(seed)
    n    = len(close)
    ret  = close.pct_change().fillna(0.0)
    vol  = ret.rolling(20).std().fillna(ret.std())
    op   = close.shift(1).fillna(close.iloc[0])
    rf   = np.abs(rng.normal(1.2, 0.5, n)).clip(0.2, 3.0)
    half = close.values * vol.values * rf
    hi   = np.maximum(op.values, close.values) + half
    lo   = np.maximum(np.minimum(op.values, close.values) - half, close.values * 0.3)
    vv   = np.abs(rng.normal(1_000_000, 300_000, n)).astype(int)
    df   = pd.DataFrame(
        {"Open": op.values, "High": hi, "Low": lo, "Close": close.values, "Volume": vv},
        index=close.index,
    )
    df["atr14"] = atr_fn(df["High"], df["Low"], df["Close"], 14)
    df["adx14"] = adx_fn(df["High"], df["Low"], df["Close"], 14)
    return df


def slice_data(data: dict, start: str, end: str) -> dict:
    return {t: df.loc[start:end] for t, df in data.items()
            if len(df.loc[start:end]) >= 60}


class MomentumRotationStrategy:
    """
    Cross-sectional momentum rotation: hold top-N coins by trailing return.
    Regime gate: BTC price > EMA(ema_period) — blocks new entries in bear phase.
    Rebalances every `rebalance_days` calendar days.
    """

    def __init__(self,
                 ema_period: int     = 150,
                 mom_lb: int         = 90,
                 min_hold_days: int  = 0,
                 rebalance_days: int = 30):
        self.ema_period     = ema_period
        self.mom_lb         = mom_lb
        self.min_hold_days  = min_hold_days
        self.rebalance_days = rebalance_days
        self.signals: dict  = {}

    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        btc_close = btc_full["Close"].ffill()

        # Regime: BTC price above long-term EMA
        btc_ema    = ema_fn(btc_close, self.ema_period)
        regime_bull = btc_close > btc_ema

        # Align all coin prices to BTC date index
        all_dates    = btc_close.index.sort_values()
        coin_prices: dict = {}
        for _, ticker, _ in COINS:
            if ticker in data:
                coin_prices[ticker] = data[ticker]["Close"].reindex(all_dates).ffill()

        sig_dict: dict     = {t: pd.Series(0, index=all_dates, dtype=int) for _, t, _ in COINS}
        in_position: dict  = {}   # ticker -> entry_date
        last_rebalance     = None

        for date in all_dates:
            # Only rebalance every rebalance_days
            if last_rebalance is not None and (date - last_rebalance).days < self.rebalance_days:
                continue
            last_rebalance = date

            bull = bool(regime_bull.loc[date]) if date in regime_bull.index else False

            if not bull:
                # Bearish regime: exit all positions where min hold is satisfied
                for t in list(in_position.keys()):
                    held = (date - in_position[t]).days
                    if held >= self.min_hold_days:
                        if date in sig_dict[t].index:
                            sig_dict[t].loc[date] = -1
                        del in_position[t]
                continue

            # Compute trailing mom_lb-day returns
            returns: dict = {}
            for ticker, prices in coin_prices.items():
                try:
                    idx = prices.index.get_loc(date)
                except KeyError:
                    continue
                start_idx = idx - self.mom_lb
                if start_idx < 0:
                    continue
                p0 = float(prices.iloc[start_idx])
                p1 = float(prices.iloc[idx])
                if p0 > 0:
                    returns[ticker] = (p1 / p0) - 1.0

            if len(returns) < N_TOP:
                continue

            # Select top-N by trailing return
            ranked    = sorted(returns.items(), key=lambda x: x[1], reverse=True)
            top_set   = set(t for t, _ in ranked[:N_TOP])

            # Exit coins that dropped out of top-N (if min hold satisfied)
            for t in list(in_position.keys()):
                if t not in top_set:
                    held = (date - in_position[t]).days
                    if held >= self.min_hold_days:
                        if date in sig_dict[t].index:
                            sig_dict[t].loc[date] = -1
                        del in_position[t]

            # Enter coins newly in top-N (respecting max N_TOP positions)
            for t in top_set:
                if t not in in_position and len(in_position) < N_TOP:
                    if date in sig_dict[t].index:
                        sig_dict[t].loc[date] = 1
                    in_position[t] = date

        self.signals = sig_dict

    def get_signals(self, ticker: str) -> pd.Series:
        return self.signals.get(ticker, pd.Series(dtype=int))


def run_is(full_data, is_data, btc_full, eth_full, sp, bt_fixed):
    strat = MomentumRotationStrategy(**sp)
    strat.prepare(full_data, btc_full, eth_full)
    bt = AdvancedBacktester(initial_capital=100_000, **bt_fixed)
    return RiskReport(bt.run(is_data, strat))


def run_oos(full_data, combined_data, btc_full, eth_full, sp, bt_fixed):
    strat  = MomentumRotationStrategy(**sp)
    strat.prepare(full_data, btc_full, eth_full)
    bt     = AdvancedBacktester(initial_capital=100_000, **bt_fixed)
    result = bt.run(combined_data, strat)
    eq     = result.equity_curve
    oos_eq = eq.loc[OOS_START:]
    oos_trades = [t for t in result.closed_trades
                  if t.exit_date is not None and t.exit_date >= pd.Timestamp(OOS_START)]
    if len(oos_eq) == 0:
        return None
    return RiskReport(AdvancedBacktestResult(
        equity_curve=oos_eq.tolist(),
        dates=oos_eq.index.tolist(),
        closed_trades=oos_trades,
        initial_capital=float(oos_eq.iloc[0]),
    ))


def main():
    print("\n" + "=" * 72)
    print("  ITERATION 46 — Cross-Sectional Momentum Rotation (Top-2 of 5)")
    print(f"  Universe: BTC, ETH, BNB, ADA, TRX  |  Hold top-{N_TOP} by trailing return")
    print("  Regime gate: BTC price > EMA(period)  |  Rebalance: monthly (30d)")
    print(f"  Tax: STCG {TAX_RATE*100:.0f}%/<365d  |  LTCG 20%/>=365d")
    print("=" * 72)

    print("\n  Fetching price data ...")
    btc_full = fetch_btc_full()
    print(f"  BTC: {len(btc_full)} rows  last=${float(btc_full['Close'].iloc[-1]):,.2f}")

    full_data: dict = {}
    for coin, label, seed in COINS:
        try:
            s = btc_full["Close"] if label == "BTC" else fetch_close(coin)
            full_data[label] = synthesize_ohlcv(s, seed=seed)
            print(f"  {label}: {len(s)} rows  last=${float(s.iloc[-1]):,.2f}")
        except Exception as e:
            print(f"  {label}: FAILED ({e})")

    if len(full_data) < 3:
        print("  Insufficient data."); return

    # Print OOS return by coin for reference
    print("\n  OOS coin returns (2021-2024):")
    for _, label, _ in COINS:
        if label in full_data:
            prices = full_data[label]["Close"].loc[OOS_START:OOS_END]
            if len(prices) > 0:
                ret = (prices.iloc[-1] / prices.iloc[0] - 1) * 100
                print(f"    {label}: {ret:+.0f}%")

    # 30-day momentum snapshot
    print("\n  Sample 90-day trailing returns at 2021-01-01 (momentum ranking):")
    for _, label, _ in COINS:
        if label in full_data:
            prices = full_data[label]["Close"]
            idx_2021 = prices.index.searchsorted(pd.Timestamp("2021-01-01"))
            if idx_2021 >= 90:
                p0 = float(prices.iloc[idx_2021 - 90])
                p1 = float(prices.iloc[idx_2021])
                print(f"    {label}: {(p1/p0-1)*100:+.0f}%")

    is_data       = slice_data(full_data, IS_START, IS_END)
    combined_data = slice_data(full_data, IS_START, OOS_END)

    bt_fixed = dict(
        max_positions=N_TOP,
        position_size_pct=1.0 / N_TOP,
        atr_stop_multiplier=20.0,
        atr_trail_multiplier=12.0,
        risk_per_trade_pct=1.0 / N_TOP,
        short_term_tax_rate=TAX_RATE,
    )

    strat_configs = [
        dict(ema_period=ema, mom_lb=lb, min_hold_days=mhd, rebalance_days=30)
        for ema in [100, 150, 200]
        for lb in [60, 90, 180]
        for mhd in [0, 180]
    ]

    print(f"\n  IS grid ({len(strat_configs)} configs, {IS_START}–{IS_END}) ...")
    print(f"  {'ema':>4} {'mom_lb':>7} {'mhd':>5}"
          f"  {'Mo%':>7} {'SR':>6} {'DD%':>8} {'N':>4} {'Hold':>5} {'score':>7}")
    print("  " + "-" * 60)

    results_is = []
    for sp in strat_configs:
        try:
            rpt = run_is(full_data, is_data, btc_full, btc_full, sp, bt_fixed)
            m   = rpt.full_metrics()
            mo  = m.get("monthly_return_pct", 0)
            sr  = m.get("sharpe_ratio", 0)
            dd  = m.get("max_drawdown_pct", 0)
            nt  = m.get("n_trades", 0)
            hld = m.get("avg_holding_days", 0) or 0
            if nt < 2:
                continue
            score = sr * 3.0 + mo * 0.5 - max(0, -40 - dd) * 0.5
            results_is.append((score, mo, sr, dd, hld, sp, m))
            print(f"  {sp['ema_period']:>4}  {sp['mom_lb']:>5}d  {sp['min_hold_days']:>5}"
                  f"  {mo:>+7.2f}%  {sr:>6.2f}  {dd:>8.2f}%  {nt:>4}  {hld:>4.0f}d  {score:>7.2f}")
        except Exception as ex:
            print(f"  [skip] {ex}")

    results_is.sort(key=lambda x: x[0], reverse=True)
    if not results_is:
        print("  No valid IS configs."); return

    best_score, best_mo, best_sr, best_dd, best_hld, best_sp, _ = results_is[0]
    print(f"\n  IS Champion: ema={best_sp['ema_period']} mom_lb={best_sp['mom_lb']} "
          f"mhd={best_sp['min_hold_days']}  score={best_score:.2f}")

    # OOS evaluation of all configs
    print(f"\n  OOS grid ({OOS_START}–{OOS_END}) ...")
    print(f"  {'ema':>4} {'mom_lb':>7} {'mhd':>5}"
          f"  {'Mo%':>7} {'SR':>6} {'DD%':>8} {'N':>4} {'Hold':>5} {'Eq$':>12}")
    print("  " + "-" * 72)

    results_oos = []
    for _, _, _, _, _, sp, _ in results_is:
        try:
            rpt = run_oos(full_data, combined_data, btc_full, btc_full, sp, bt_fixed)
            if rpt is None:
                continue
            m   = rpt.full_metrics()
            mo  = m.get("monthly_return_pct", 0)
            sr  = m.get("sharpe_ratio", 0)
            dd  = m.get("max_drawdown_pct", 0)
            nt  = m.get("n_trades", 0)
            hld = m.get("avg_holding_days", 0) or 0
            eq  = m.get("final_equity", 0)
            results_oos.append((mo, sr, dd, nt, hld, eq, sp, m))
            print(f"  {sp['ema_period']:>4}  {sp['mom_lb']:>5}d  {sp['min_hold_days']:>5}"
                  f"  {mo:>+7.2f}%  {sr:>6.2f}  {dd:>8.2f}%  {nt:>4}  {hld:>4.0f}d  ${eq:>12,.0f}")
        except Exception as ex:
            print(f"  [skip] {ex}")

    if not results_oos:
        print("  No OOS results."); return

    # IS-champion OOS result (walk-forward blind test)
    is_champ_oos = None
    for row in results_oos:
        if row[6] == best_sp:
            is_champ_oos = row
            break
    if is_champ_oos is None:
        is_champ_oos = results_oos[0]  # fallback

    oos_mo, oos_sr, oos_dd, oos_n, oos_hld, oos_eq, oos_sp, oos_m = is_champ_oos

    # Tax adjustment
    ltcg_rate    = 0.20
    stcg_rate    = TAX_RATE
    gross_mo     = oos_mo
    if oos_hld >= 365:
        net_mo   = gross_mo * (1 - ltcg_rate) / (1 - stcg_rate)
        tax_note = f"LTCG {ltcg_rate*100:.0f}% (hold {oos_hld:.0f}d)"
    else:
        net_mo   = gross_mo
        tax_note = f"STCG {stcg_rate*100:.0f}% (hold {oos_hld:.0f}d) — already net"

    oos_wr = oos_m.get("win_rate_pct", 0)
    oos_pf = oos_m.get("profit_factor", 0)
    oos_cal = oos_m.get("calmar_ratio", 0)

    # Full risk report
    try:
        rpt_full = run_oos(full_data, combined_data, btc_full, btc_full, best_sp, bt_fixed)
        if rpt_full:
            rpt_full.print_report(
                title=f"OOS CHAMPION — Momentum Rotation  ema={best_sp['ema_period']}  "
                      f"mom_lb={best_sp['mom_lb']}  mhd={best_sp['min_hold_days']}  "
                      f"[STCG {TAX_RATE*100:.0f}%, {OOS_START}–{OOS_END}]",
                period=f"{OOS_START}–{OOS_END}"
            )
    except Exception as e:
        print(f"  [report err] {e}")

    # Walk-forward verdict
    pass_criteria = [
        ("Monthly >= 2.0%",  net_mo >= 2.0,   net_mo - 2.0),
        ("Sharpe >= 1.0",    oos_sr >= 1.0,   oos_sr - 1.0),
        ("MaxDD > -40%",     oos_dd > -40.0,  oos_dd + 40.0),
        ("Calmar >= 0.8",    oos_cal >= 0.8,  oos_cal - 0.8),
        ("Win Rate >= 40%",  oos_wr >= 40.0,  oos_wr - 40.0),
        ("Profit Factor >= 1.3", oos_m.get("profit_factor", 0) >= 1.3,
         oos_m.get("profit_factor", 0) - 1.3),
        ("N Trades >= 5",    oos_n >= 5,      oos_n - 5),
    ]
    n_pass = sum(1 for _, p, _ in pass_criteria if p)

    print(f"\n{'='*72}")
    print(f"  VERDICT  —  OOS {OOS_START}–{OOS_END}, Momentum Rotation (Top-{N_TOP})")
    print(f"{'='*72}")
    for label, passed, delta in pass_criteria:
        mark = "[PASS]" if passed else "[FAIL]"
        print(f"    {mark}  {label:<25} {delta:>+.2f}")
    print(f"\n  {n_pass}/7 criteria pass")
    print(f"  Model equity: ${oos_eq:,.0f}  (OOS base = $100,000)")
    print(f"  Avg hold: {oos_hld:.0f} days  ({tax_note})")
    print(f"\n  IS-champion OOS (walk-forward blind): {oos_mo:+.2f}%/mo  "
          f"Sharpe {oos_sr:.2f}  MaxDD {oos_dd:.2f}%")
    print(f"  After-tax net ({tax_note}): ~{net_mo:+.2f}%/month")

    if n_pass >= 7:
        adj = "(1-LTCG)/(1-STCG)" if oos_hld >= 365 else "STCG already"
        print(f"\n  *** PASS 7/7 — Momentum Rotation qualifies! ***")
        print(f"  Net after-tax: ~{net_mo:+.2f}%/month  [{adj}]")
    else:
        print(f"\n  --- {n_pass}/7 pass — does not meet all criteria ---")


if __name__ == "__main__":
    main()
