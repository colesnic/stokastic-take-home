"""
Iteration 52: Asymmetric MVRV+AdrActCnt Entry / Price EMA Exit
===============================================================
Decouples entry and exit conditions to get best of both worlds:
  ENTRY:  MVRV EMA trend + AdrActCnt EMA trend + vol gate  (no price EMA)
          → earlier entries, captures more bull run upside (like Iter 50)
  EXIT:   BTC price < EMA only                              (fast protection)
          → quick exit on price breakdown, reduces MaxDD (like Iter 51)

Prior results:
  Iter 50 (MVRV+AdrActCnt entry & exit): +3.93%/mo after-LTCG, MaxDD -42% FAIL
  Iter 51 (Price+MVRV+AdrActCnt entry & exit): +3.62%/mo after-LTCG, MaxDD -26% PASS

Hypothesis: asymmetric design captures Iter 50's upside with Iter 51's downside protection.
...
class AsymmetricMVRVStrategy:
    """
    Entry: MVRV EMA trend + AdrActCnt EMA trend + vol gate (no price EMA).
    Exit:  BTC price < EMA only (fast exit on price breakdown).
    Asymmetric design: early entry, protected exit.
    """
...
def run_is(full_data,is_data,btc_full,eth_full,sp,bt_fixed):
    strat=AsymmetricMVRVStrategy(**sp); strat.prepare(full_data,btc_full,eth_full)
...
def main():
    print("\n"+"="*72)
    print("  ITERATION 52 — Asymmetric MVRV Entry / Price EMA Exit + Vol Gate")
    print("  ENTRY: MVRV+AdrActCnt trend (no price EMA — earlier entries)")
    print("  EXIT:  BTC price < EMA only (fast protection — lower MaxDD)")
    ...
    print(f"\n{'='*72}")
    print(f"  VERDICT  —  OOS {OOS_START}–{OOS_END}, Asymmetric MVRV Entry/Price Exit")
    print(f"{'='*72}")
    ...
    print(f"\n  Iter 41: +3.00% → +3.69%/mo LTCG | Iter 50: +3.19% → +3.93% (MaxDD FAIL)")
    print(f"  Iter 51: +2.94% → +3.62%/mo LTCG | Iter 52: {oos_mo:+.2f}% → {net_mo:+.2f}%/mo after-tax")
    if n_pass>=7:
        print(f"\n  *** PASS 7/7 ***")
        if net_mo>3.69: print(f"  *** BEATS ITER 41! Net {net_mo:+.2f}%/mo ***")
    else: print(f"\n  --- {n_pass}/7 pass ---")
