"""
Iteration 51: BTC Price + MVRV + AdrActCnt Triple Regime + Vol Gate
====================================================================
Extends Iter 50 (MVRV+AdrActCnt dual, 6/7 PASS, +3.93%/mo after-LTCG)
by adding BTC price > EMA as the third condition to reduce MaxDD.

Iter 50 result:  +3.19%/mo, Sharpe 1.26, MaxDD -42.03% (FAILS MaxDD)
Fix target:      MaxDD > -40% by adding price filter as extra guard

Triple regime (all BTC-native, no ETH dependency):
  1. BTC price > EMA(ema_period)   — trend filter
  2. BTC CapMVRVCur EMA(short) > EMA(long)  — valuation expansion
  3. BTC AdrActCnt  EMA(short) > EMA(long)  — network activity
...
class PriceMVRVAdrStrategy:
    """Triple regime: BTC price EMA + MVRV EMA trend + AdrActCnt EMA trend. Vol gate."""
...
def run_is(full_data,is_data,btc_full,eth_full,sp,bt_fixed):
    strat=PriceMVRVAdrStrategy(**sp); strat.prepare(full_data,btc_full,eth_full)
...
def main():
    print("\n"+"="*72)
    print("  ITERATION 51 — BTC Price+MVRV+AdrActCnt Triple Regime + Vol Gate")
    print("  Adds BTC price > EMA to Iter 50 dual-regime to reduce MaxDD")
    ...
    print(f"\n{'='*72}")
    print(f"  VERDICT  —  OOS {OOS_START}–{OOS_END}, Price+MVRV+AdrActCnt Triple Regime")
    print(f"{'='*72}")
    ...
    print(f"\n  Iter 41 baseline: +3.00%/mo → +3.69%/mo after-LTCG  |  Sharpe 1.22  |  MaxDD -25.83%")
    print(f"  Iter 50 result:   +3.19%/mo → +3.93%/mo after-LTCG  |  Sharpe 1.26  |  MaxDD -42.03% FAIL")
    print(f"  Iter 51 result:   {oos_mo:+.2f}%/mo → {net_mo:+.2f}%/mo after-tax  |  Sharpe {oos_sr:.2f}  |  MaxDD {oos_dd:.2f}%")
    if n_pass>=7:
        print(f"\n  *** PASS 7/7 — Price+MVRV+AdrActCnt qualifies! ***")
