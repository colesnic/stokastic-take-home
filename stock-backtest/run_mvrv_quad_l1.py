"""
Iteration 17: MVRV Momentum × Quad L1 — BTC+ETH+BNB+ADA
=========================================================
Replaces active-address confirmation (Iter 15) with MVRV momentum:
  Signal = BTC price > EMA  AND  MVRV_short_EMA > MVRV_long_EMA

MVRV = Market Cap / Realized Cap = "market value vs what investors actually paid"
MVRV momentum rising = more holders becoming on-chain profitable → bullish sentiment
MVRV momentum falling = loss realization spreading → bearish

Why MVRV momentum is structurally different from active addresses:
  Active addresses: measures USER ACTIVITY (anyone can inflate with spam tx)
  MVRV: measures PROFITABILITY STATE of all BTC holders
    → Cannot be gamed or inflated
    → Directly reflects the economic incentive to hold vs sell

Key empirical advantage over active addresses:
  2022 crash: MVRV ema30>ema90 for 0% of days (perfect bear avoidance!)
              Active addresses grew +20.7% in 2022 (false bullish signal)
  2023 recovery: MVRV ema30>ema90 for 78% of days (captures recovery)
  2018 crash: MVRV ema30>ema90 for only 2% of days (IS crash avoided)

Universe: BTC+ETH+BNB+ADA (25% each) — same as Iter 15

Data source: Coinmetrics CapMVRVCur column in btc.csv
...
def fetch_btc_full() -> pd.DataFrame:
    ...
    cols = ["Date", "Close", "MVRV", "AdrActCnt"]
...
class MvrvQuadL1Strategy:
    """
    BTC price EMA + BTC MVRV momentum → hold BTC+ETH+BNB+ADA equally.
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame) -> None:
        ...
        regime_bull  = price_bull & mvrv_bull
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 17 — MVRV Momentum × Quad L1: BTC+ETH+BNB+ADA")
    print("  Signal: BTC price > EMA  AND  MVRV 30d > 90d EMA")
    print(f"  Tax: STCG {TAX_RATE*100:.0f}%/<365d  |  LTCG 20%/≥365d")
    print("=" * 72)

    print("\n  Fetching BTC (with MVRV), ETH, BNB, ADA ...")
    try:
    ...
    # MVRV momentum signal statistics
    mvrv = btc_full["MVRV"].ffill()
    print(f"\n  MVRV momentum signal (30d EMA vs 90d EMA):")
    print(f"  {'Year':<10}  {'BTC%':>8}  {'MVRV avg':>9}  {'Bull%':>8}  {'Start':>6}  {'End':>6}")
    mvrv_ema30 = ema_fn(mvrv, 30)
    mvrv_ema90 = ema_fn(mvrv, 90)
    mvrv_bull  = mvrv_ema30 > mvrv_ema90
    for yr in ["2018","2019","2020","2021","2022","2023","2024"]:
        s = btc_full[btc_full.index.year == int(yr)].dropna(subset=["Close","MVRV"])
        if len(s) > 1:
            btc_ret = (float(s["Close"].iloc[-1])/float(s["Close"].iloc[0])-1)*100
            mv = mvrv[mvrv.index.year == int(yr)]
            bull_frac = mvrv_bull[mvrv_bull.index.year == int(yr)].mean() * 100
            print(f"  {yr:<10}  {btc_ret:>+7.1f}%  {mv.mean():>9.2f}  {bull_frac:>7.0f}%  "
                  f"{float(mv.iloc[0]):>5.2f}  {float(mv.iloc[-1]):>5.2f}")
    ...
    strat_configs = [
        dict(ema_period=ema, mvrv_short=ms, mvrv_long=ml, min_hold_days=mhd,
             rebalance_days=7)
        for ema in [100, 150, 200]
        for (ms, ml) in [(20, 60), (30, 90), (45, 120)]
        for mhd in [0, 365]
    ]

    print(f"\n  IS grid search ({len(strat_configs)} configs, {IS_START}–{IS_END}) ...")
    print(f"  {'ema':>4} {'mvs/mvl':>8} {'mhd':>4}  {'Mo%':>7} {'SR':>6} {'DD%':>8} {'N':>4} {'Hold':>5} {'score':>7}")
    print("  " + "-" * 62)
    ...
    for sp in strat_configs:
        try:
            ...
            print(f"  {sp['ema_period']:>4} {mv_str:>8} {sp.get('min_hold_days',0):>4}"
    ...
    print(f"\n  IS champion: EMA{best_sp['ema_period']} mvrv={mv_str} mhd={best_sp.get('min_hold_days',0)}")
    ...
    print(f"  {'ema':>4} {'mvs/mvl':>8} {'mhd':>4}  {'Mo%':>8} {'SR':>6} {'DD%':>8} {'N':>4} {'Hold':>5} {'Eq$':>12}")
    print("  " + "-" * 74)
    ...
    mv_str = f"{champ_sp['mvrv_short']}/{champ_sp['mvrv_long']}"
    champ_rpt.print_full_report(
        f"OOS CHAMPION — MVRV Momentum × Quad L1  ema={champ_sp['ema_period']}"
        f"  mvrv={mv_str}  mhd={champ_sp.get('min_hold_days',0)}"
    ...
    print(f"\n{'='*72}")
    print("  VERDICT  —  OOS 2021–2024, MVRV Momentum × Quad L1")
    print(f"{'='*72}")
    criteria = [
    ...
    if is_champ_oos:
        ...
        mv_str = f"{isp['mvrv_short']}/{isp['mvrv_long']}"
