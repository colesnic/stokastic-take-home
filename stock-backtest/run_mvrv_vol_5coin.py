"""
Iteration 42: BTC MVRV Trend + Vol Gate Combined — 5-Coin Portfolio
====================================================================
Combines the two strongest entry signals:
  - Iter 34/40: BTC MVRV trend (EMA_short > EMA_long) — market expansion phase
  - Iter 27/41: BTC vol < threshold — stable/accumulation conditions

HYPOTHESIS:
  Both signals block Jan 2018 and allow Jan 2021.
  Together they may produce tighter, higher-quality entry windows.
  Fewer entries but better-timed → higher win rate and Sharpe.

MVRV trend:
  Entry allowed when EMA(CapMVRVCur, short) > EMA(CapMVRVCur, long)
  = market's aggregate profit expanding = bull cycle

Vol gate:
  Entry allowed when BTC 30d realized vol < threshold
  = calm accumulation, not explosive/unstable frenzy

Combined design:
  Entry: regime_bull AND mvrv_rising AND vol < vol_threshold
  Exit:  regime_bull = False (neither MVRV nor vol triggers exits — preserves LTCG)

Key dates:
  Jan 2018: MVRV FALLING + vol=126% → DOUBLE BLOCK ✗✗ (ICO bubble crash)
  Oct 2020: MVRV RISING + vol=42% → DOUBLE ALLOW ✓✓ (post-halving accumulation)
  Jan 2021: MVRV RISING + vol=51% → DOUBLE ALLOW ✓✓ (DeFi bull start)
  Jun 2021: MVRV RISING + vol=85% → vol BLOCKS (China ban crash) ✓
  Jan 2022: MVRV FALLING → regime exits anyway, but extra protection ✓
  Jan 2024: MVRV RISING + vol=38% → DOUBLE ALLOW ✓✓ (ETF-era bull)

Portfolio: BTC, ETH, BNB, ADA, TRX (5-coin, same as Iter 40/41)
...
def fetch_btc_full() -> pd.DataFrame:
    ...
    return df[["Date","Close","AdrActCnt","CapMVRVCur"]].dropna(subset=["Close"]).set_index("Date").sort_index()
...
class MVRVVolCombinedStrategy:
    """
    Triple regime (BTC price + AdrActCnt + ETH TxCnt) with BOTH:
      1. BTC MVRV trend (EMA_short > EMA_long) as entry-only gate
      2. BTC realized vol < threshold as entry-only gate
    Entry: regime_bull AND mvrv_rising AND vol < vol_threshold
    Exit:  regime bearish (neither signal triggers exit — preserves LTCG)
    """

    def __init__(self,
                 ema_period: int      = 100,
                 act_short: int       = 20,  act_long: int = 60,
                 mvrv_short: int      = 10,  mvrv_long: int = 30,
                 vol_lookback: int    = 30,
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        # MVRV trend gate
        mvrv      = btc_full["CapMVRVCur"].ffill()
        mvrv_s    = ema_fn(mvrv, self.mvrv_short)
        mvrv_l    = ema_fn(mvrv, self.mvrv_long)
        mvrv_ok   = mvrv_s > mvrv_l  # MVRV expanding = bull cycle
        ...
        # Combined: both must be true for entry
        entry_allowed = regime_bull & mvrv_ok & vol_ok
...
def run_is(full_data, is_data, btc_full, eth_full, sp, bt_fixed):
    strat = MVRVVolCombinedStrategy(**sp)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 42 — MVRV Trend + Vol Gate Combined (5-Coin)")
    print("  Entry: regime_bull AND mvrv_rising AND vol < threshold")
    print("  Exit: regime_bull = False only (neither triggers exits)")
    print(f"  Tax: STCG {TAX_RATE*100:.0f}%/<365d  |  LTCG 20%/≥365d")
    print("=" * 72)

    print("\n  Fetching BTC (price + AdrActCnt + MVRV) and ETH (price + TxCnt) ...")
    ...
    mvrv_s  = ema_fn(mvrv, 10)
    mvrv_l  = ema_fn(mvrv, 30)

    print("\n  Combined signal diagnostics (mvrv_ema=10/30, vol_thr=0.80):")
    for date_str in ["2018-01-01", "2020-09-01", "2021-01-01",
                     "2021-06-01", "2022-01-01", "2024-01-01"]:
        try:
            dt = pd.Timestamp(date_str)
            idx = btc_full.index.asof(dt)
            mv = float(mvrv.loc[idx])
            ms = float(mvrv_s.loc[idx])
            ml = float(mvrv_l.loc[idx])
            vl = float(vol30.loc[idx])
            mvrv_ok = ms > ml
            vol_ok  = vl < 0.80
            state   = "ALLOW ✓✓" if (mvrv_ok and vol_ok) else (
                "BLOCK ✗✗" if (not mvrv_ok and not vol_ok) else
                f"PARTIAL ({'MVRV✓' if mvrv_ok else 'MVRV✗'},{'VOL✓' if vol_ok else 'VOL✗'})"
            )
            print(f"  {date_str}: MVRV={mv:.2f} (ema {ms:.2f}/{ml:.2f}) vol={vl:.2f} → {state}")
        except Exception:
            pass
    ...
    print(f"  {'ema':>4} {'act':>7} {'mvrv':>8} {'vt':>6} {'mhd':>4}"
    ...
    for sp in strat_configs:
        try:
            ...
            print(f"  {sp['ema_period']:>4} {sp['act_short']}/{sp['act_long']:>3}"
                  f"  {sp['mvrv_short']}/{sp['mvrv_long']:>2}"
    ...
    print(f"\n  IS champion: EMA{best_sp['ema_period']}"
          ...
          f" mvrv={best_sp['mvrv_short']}/{best_sp['mvrv_long']}"
    ...
    champ_rpt.print_full_report(
        f"OOS CHAMPION — MVRV+Vol 5-Coin"
        f"  ema={champ_sp['ema_period']}"
        f"  mvrv={champ_sp['mvrv_short']}/{champ_sp['mvrv_long']}"
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021–2024, MVRV Trend + Vol Gate Combined (5-Coin)")
    print("=" * 72)
    ...
    print(f"  After-tax net ({'LTCG 20%' if hd_is >= 365 else 'STCG 35%'}): ~{ltcg_adj:+.2f}%/month")
