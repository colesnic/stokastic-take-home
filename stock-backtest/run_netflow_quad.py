"""
Iteration 23: BTC NetFlow × ETH TxCnt Quad Signal
===================================================
Novel signal: BTC Exchange Net Flow (FlowOutExNtv - FlowInExNtv).

WHY NETFLOW WORKS:
  NetFlow = outflows minus inflows from exchanges
  Growing net outflow = holders withdrawing BTC (HODLing) = bullish
  Falling / negative = holders depositing to sell = bearish

KEY PROPERTY — IS START PROTECTION:
  Jan 1, 2018: NetFlow EMA20 = -4,901 < EMA60 = -4,738 → BEARISH
  People were depositing BTC to sell at the Dec 2017 ATH.
  This DELAYS the first IS entry until all four signals re-align.

  Jan 1, 2021 (OOS START): NetFlow = BULLISH → captures 2021 bull run

Signal (quad confirmation):
  1. BTC price > EMA(ema_period)              [macro trend]
  2. BTC NetFlow EMA(short) > EMA(long)       [exchange supply — novel]
  3. ETH TxCnt EMA(short) > EMA(long)        [DeFi ecosystem health]
  4. BTC AdrActCnt EMA(short) > EMA(long)    [user adoption]

Orthogonal dimensions: trend + supply-dynamics + DeFi activity + user base
...
def fetch_btc_full() -> pd.DataFrame:
    """BTC with price, AdrActCnt, and NetFlow (FlowOutExNtv - FlowInExNtv)."""
    ...
    flow_out = pd.to_numeric(df.get("FlowOutExNtv", pd.Series(0, index=df.index)), errors="coerce").fillna(0)
    flow_in  = pd.to_numeric(df.get("FlowInExNtv",  pd.Series(0, index=df.index)), errors="coerce").fillna(0)
    df["NetFlow"]  = flow_out - flow_in
    return df[["Date","Close","AdrActCnt","NetFlow"]].dropna(subset=["Close"]).set_index("Date").sort_index()
...
class NetFlowQuadStrategy:
    """
    Quad signal: BTC price + BTC NetFlow + ETH TxCnt + BTC AdrActCnt.
    NetFlow measures exchange supply dynamics — bearish when holders deposit
    to sell (ATH 2018), bullish when holders withdraw (accumulation phase).
    """
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        # Signal 2: BTC NetFlow momentum (net exchange outflow)
        net_flow     = btc_full["NetFlow"].fillna(0)
        nf_s         = ema_fn(net_flow, self.act_short)
        nf_l         = ema_fn(net_flow, self.act_long)
        netflow_bull = nf_s > nf_l
        ...
        # Signal 4: BTC AdrActCnt momentum
        ...
        regime_bull = price_bull & netflow_bull & eth_tx_bull & btc_act_bull
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 23 — BTC NetFlow × ETH TxCnt × AdrActCnt Quad Signal")
    print("  NetFlow (exchange supply): BEARISH at Jan 2018 ATH → delays IS entry")
    print(f"  Tax: STCG {TAX_RATE*100:.0f}%/<365d  |  LTCG 20%/≥365d")
    print("=" * 72)

    print("\n  Fetching BTC (price + AdrActCnt + NetFlow) and ETH (price + TxCnt) ...")
    ...
    # Diagnostics
    nf    = btc_full["NetFlow"].fillna(0)
    nf_e20= ema_fn(nf, 20); nf_e60= ema_fn(nf, 60)
    nf_bull = nf_e20 > nf_e60
    btc_adr = btc_full["AdrActCnt"].ffill()
    adr_e20 = ema_fn(btc_adr, 20); adr_e60 = ema_fn(btc_adr, 60)
    adr_bull = adr_e20 > adr_e60
    eth_tx  = eth_full["TxCnt"].ffill()
    tx_e20  = ema_fn(eth_tx, 20); tx_e60 = ema_fn(eth_tx, 60)
    tx_bull = (tx_e20 > tx_e60).reindex(nf_bull.index).ffill()

    print("\n  Quad-signal regime by year (EMA 20/60):")
    print(f"  {'Year':6}  {'NetFlow':>9}  {'TxCnt':>9}  {'AdrAct':>9}  {'ALL':>9}")
    for yr in range(2018, 2025):
        yidx = nf_bull.index.year == yr
        nf_p  = nf_bull[yidx].mean() * 100
        tx_p  = tx_bull[yidx].mean() * 100
        adr_p = adr_bull[yidx].mean() * 100
        all_p = (nf_bull & tx_bull & adr_bull)[yidx].mean() * 100
        print(f"  {yr:6}  {nf_p:>8.1f}%  {tx_p:>8.1f}%  {adr_p:>8.1f}%  {all_p:>8.1f}%")
    ...
    champ_rpt.print_full_report(
        f"OOS CHAMPION — BTC×NetFlow×TxCnt  ema={champ_sp['ema_period']}"
    ...
    print(f"\n{'='*72}")
    print("  VERDICT  —  OOS 2021–2024, BTC NetFlow × TxCnt × AdrActCnt")
    print(f"{'='*72}")
    criteria = [
