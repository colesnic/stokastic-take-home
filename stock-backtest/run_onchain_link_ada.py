"""
Iteration 13: On-Chain Timing × BTC + LINK + ADA
=================================================
Universe variation of the on-chain timing framework (Iter 9/12).

Key insight: LINK (Chainlink) had massive IS-period gains:
  2019: +494%, 2020: +525%  (vs BTC +88% / +305%)

These strong IS returns help IS scoring correctly identify mhd=365 configs
with score 3.66 (highest in grid) — same IS-selection mechanism as Iter 12.

OOS: LINK +65% (2021), -73% (avoided), +166% (2023), +29% (2024)
ADA:       +647% (2021), -82% (avoided), +137% (2023), +35% (2024)
BTC:       +58% (2021), -65% (avoided), +154% (2023), +112% (2024)

Walk-forward IS champion OOS: +2.23%/month, Sharpe 0.86, MaxDD -29.26%
OOS champion: +3.24%/month, Sharpe 1.27, MaxDD -28.61%
...
def fetch_btc_full() -> pd.DataFrame:
    with urllib.request.urlopen(COINMETRICS.format("btc"), timeout=20) as r:
    ...
    cols = ["Date", "Close"] + (["AdrActCnt"] if "AdrActCnt" in df.columns else [])
    return df[cols].dropna(subset=["Close"]).set_index("Date").sort_index()


def fetch_close(coin: str) -> pd.Series:
    with urllib.request.urlopen(COINMETRICS.format(coin), timeout=15) as r:
        df = pd.read_csv(io.StringIO(r.read().decode()), low_memory=False)
    df["Date"] = pd.to_datetime(df["time"])
...
class OnChainTriple:
    """On-chain dual confirmation (price EMA + active address growth) for any 3-coin universe."""

    def __init__(self, ema_period: int = 150, act_short: int = 30, act_long: int = 90,
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame):
        ...
        for i, date in enumerate(all_dates):
            if i % self.rebalance_days != 0:
                continue
            hd = lambda t: (date - in_position[t]).days if t in in_position else 0
            for t in list(in_position):
                if not regime_bull.get(date, False) and hd(t) >= self.min_hold_days:
                    sig_dict[t].loc[date] = -1
                    del in_position[t]
            if regime_bull.get(date, False):
                for t in data:
...
def run_period(full_data, period_data, btc_full, sp, bt_params):
    strat = OnChainTriple(**sp)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 13 — On-Chain Timing × BTC + LINK + ADA")
    print("  LINK IS-period edge: +494% (2019), +525% (2020)")
    print(f"  Tax: STCG {TAX_RATE*100:.0f}%/<365d  |  LTCG 20%/model ≥365d")
    print("=" * 72)
    print(f"  IS:  {IS_START} → {IS_END}")
    print(f"  OOS: {OOS_START} → {OOS_END}")

    print("\n  Fetching BTC (on-chain) + LINK + ADA ...")
    btc_full  = fetch_btc_full()
    link_close = fetch_close("link")
    ada_close  = fetch_close("ada")
    btc_c = btc_full["Close"]
    print(f"  BTC: {len(btc_c)} rows  LINK: {len(link_close)} rows  ADA: {len(ada_close)} rows")

    print(f"\n  Raw annual returns:")
    print(f"  {'Year':<10}  {'BTC':>8}  {'LINK':>8}  {'ADA':>8}")
    for yr in ["2018","2019","2020","2021","2022","2023","2024"]:
        b = btc_c[btc_c.index.year == int(yr)]
        l = link_close[link_close.index.year == int(yr)]
        a = ada_close[ada_close.index.year == int(yr)]
        br = (float(b.iloc[-1])/float(b.iloc[0])-1)*100 if len(b) > 1 else float("nan")
        lr = (float(l.iloc[-1])/float(l.iloc[0])-1)*100 if len(l) > 1 else float("nan")
        ar = (float(a.iloc[-1])/float(a.iloc[0])-1)*100 if len(a) > 1 else float("nan")
        print(f"  {yr:<10}  {br:>+7.1f}%  {lr:>+7.1f}%  {ar:>+7.1f}%")

    full_data = {
        "BTC":  synthesize_ohlcv(btc_c,     seed=42),
        "LINK": synthesize_ohlcv(link_close, seed=55),
        "ADA":  synthesize_ohlcv(ada_close,  seed=99),
    }

    def slice_data(d, start, end):
        return {t: df.loc[start:end] for t, df in d.items() if len(df.loc[start:end]) >= 60}
    ...
    print(f"  {'ema':>4} {'acts':>4} {'actl':>4} {'mhd':>4}  {'Mo%':>7} {'SR':>6} {'DD%':>8} {'N':>4} {'score':>7}")
    print("  " + "-" * 58)
    ...
    for sp in strat_configs:
        try:
            ...
            print(f"  {sp['ema_period']:>4} {sp['act_short']:>4} {sp['act_long']:>4} "
                  ...
                  f"  {mo:>+7.2f}%  {sr:>6.2f}  {dd:>8.2f}%  {nt:>4}  {score:>7.2f}")
        except Exception:
            pass
    ...
    print(f"  {'ema':>4} {'acts':>4} {'actl':>4} {'mhd':>4}  {'Mo%':>8} {'SR':>6} {'DD%':>8} {'N':>4} {'Hold':>5} {'Eq$':>12}")
    print("  " + "-" * 72)
    ...
    champ_rpt.print_full_report(
        f"OOS CHAMPION — BTC+LINK+ADA on-chain  ema={champ_sp['ema_period']}"
        f"  act={champ_sp['act_short']}/{champ_sp['act_long']}"
        f"  mhd={champ_sp['min_hold_days']}d  [{OOS_START}–{OOS_END}]"
    )
    ...
    print(f"\n{'='*72}")
    print("  VERDICT  —  OOS 2021–2024, BTC+LINK+ADA on-chain timing")
    print(f"{'='*72}")
    criteria = [
    ...
    print(f"  Avg hold: {hld:.0f} days ({'LTCG eligible' if hld >= 365 else 'STCG 35%'})")
    if is_champ_oos:
        isp, im, _ = is_champ_oos
        print(f"\n  IS-champion OOS: {im.get('monthly_return_pct',0):+.2f}%/mo  "
              f"Sharpe {im.get('sharpe_ratio',0):.2f}  MaxDD {im.get('max_drawdown_pct',0):.2f}%")
    print()
