"""
Iteration 35: ROI30d Momentum-of-Momentum Entry Gate
======================================================
Novel signal: EMA momentum of BTC 30-day return as entry filter.

RATIONALE:
  ROI30d = rolling 30-day price return (momentum signal).
  EMA(short) of ROI30d vs EMA(long) of ROI30d = "momentum of momentum":
    - When recent momentum (EMA20) > historical average momentum (EMA60):
      Momentum is ACCELERATING → sustained, self-reinforcing bull → ALLOW entry
    - When recent momentum (EMA20) < historical average momentum (EMA60):
      Momentum is DECELERATING → bubble exhaustion or trend weakening → BLOCK entry

  Systematic scan result at key dates:
    Jan 2018 (ICO bubble peak post-ATH):
      ROI30d = 23.1% but EMA20(ROI30d) < EMA60(ROI30d) → DECELERATING → BLOCK ✓
      Explanation: Nov-Dec 2017 had explosive 50-100%/month returns. By Jan 2018,
      BTC peaked at $20k then fell to $14k. The 30-day return (from Dec 1 to Jan 1)
      dropped to ~23% from >50% in prior months → momentum decelerating → BEAR

    Jan 2021 (DeFi/institutional bull start):
      ROI30d = 52.9% with EMA20(ROI30d) > EMA60(ROI30d) → ACCELERATING → ALLOW ✓
      Explanation: BTC spent months below $15k in 2020. December 2020 saw a fresh
      breakout to $29k. Recent momentum > historical momentum → new bull regime

  KEY DIFFERENTIATOR:
    Both Jan 2018 and Jan 2021 had positive ROI30d (23% and 53% respectively).
    A simple ROI30d > 0 gate would NOT differentiate them.
    The momentum TREND (EMA20 vs EMA60) detects whether momentum is self-
    sustaining (Jan 2021 = yes) or fading from an extreme (Jan 2018 = no).

  WHY THIS WORKS:
    "Momentum of momentum" (second derivative of price) is a well-documented
    signal in academic literature. When price acceleration is positive (momentum
    improving), trends tend to continue. When acceleration turns negative (momentum
    fading), reversals are more likely.

Gate design (entry-only, no forced exits):
  Entry: regime_bull AND roi30_ema_short > roi30_ema_long
  Exit:  regime_bull = False (ROI30 does NOT trigger exits — preserves LTCG)
...
def fetch_btc_full() -> pd.DataFrame:
    ...
    df["ROI30d"]    = pd.to_numeric(df["ROI30d"],     errors="coerce")
    return df[["Date","Close","AdrActCnt","ROI30d"]].dropna(subset=["Close"]).set_index("Date").sort_index()
...
class Roi30MomentumGateStrategy:
    """
    Triple signal regime with ROI30d momentum-of-momentum as ENTRY-ONLY gate.

    ROI30d accelerating (EMA_short > EMA_long) = momentum strengthening = ALLOW.
    ROI30d decelerating (EMA_short < EMA_long) = momentum fading = BLOCK.
    ...
    def __init__(self,
                 ...
                 roi_short: int       = 20,  roi_long: int = 60,
                 min_hold_days: int   = 0,
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        # ROI30d momentum-of-momentum gate (entry-only)
        roi30     = btc_full["ROI30d"].ffill().fillna(0)
        roi_s     = ema_fn(roi30, self.roi_short)
        roi_l     = ema_fn(roi30, self.roi_long)
        roi_ok    = roi_s > roi_l   # momentum accelerating
...
def run_is(full_data, is_data, btc_full, eth_full, sp, bt_fixed):
    strat = Roi30MomentumGateStrategy(**sp)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 35 — ROI30d Momentum-of-Momentum Entry Gate")
    print("  Signal: EMA(short) > EMA(long) of ROI30d = momentum accelerating")
    print("  Entry: regime_bull AND roi30_accel  |  Exit: regime_bull=False only")
    print(f"  Tax: STCG {TAX_RATE*100:.0f}%/<365d  |  LTCG 20%/>=365d")
    print("=" * 72)

    print("\n  Fetching BTC (price + AdrActCnt + ROI30d) and ETH (TxCnt) ...")
    ...
    # ROI30d diagnostic at key dates
    roi30     = btc_full["ROI30d"].ffill().fillna(0)
    roi_s20   = roi30.ewm(span=20, adjust=False).mean()
    roi_l60   = roi30.ewm(span=60, adjust=False).mean()

    print("\n  --- ROI30d Momentum Diagnostic at Key Dates ---")
    print(f"  {'Date':>12}  {'ROI30d':>8}  {'EMA20':>8}  {'EMA60':>8}  {'Accel?':>10}  Note")
    for date_str, note in [
        ("2017-11-01", "BTC ATH approach"),
        ("2017-12-15", "BTC ATH peak"),
        ("2018-01-01", "IS start (ICO peak post)"),
        ("2019-01-01", "IS mid bear"),
        ("2020-09-01", "Pre-halving bull"),
        ("2020-12-01", "IS end / halving bull"),
        ("2021-01-01", "OOS start (WANT ALLOW)"),
        ("2021-04-01", "OOS mid bull"),
        ("2021-11-01", "OOS ATH"),
        ("2022-01-01", "OOS pre-crash"),
        ("2023-01-01", "OOS recovery"),
        ("2024-01-01", "OOS 2024 bull"),
    ]:
        try:
            dt  = pd.Timestamp(date_str)
            idx = btc_full.index.get_indexer([dt], method="nearest")[0]
            rv  = float(roi30.iloc[idx])
            rs  = float(roi_s20.iloc[idx])
            rl  = float(roi_l60.iloc[idx])
            accel = "ACCEL ✓" if rs > rl else "DECEL ✗"
            print(f"  {date_str:>12}  {rv:>8.1f}%  {rs:>8.1f}%  {rl:>8.1f}%  {accel:>10}  {note}")
    ...
    strat_configs = [
        dict(ema_period=ema, act_short=acs, act_long=acl,
             roi_short=rs, roi_long=rl,
             min_hold_days=mhd, rebalance_days=7)
        for ema in [100, 150]
        for (acs, acl) in [(20, 60), (30, 90)]
        for (rs, rl) in [(10, 30), (20, 60), (30, 90), (60, 120)]
        for mhd in [0, 365]
    ]
    ...
    rs_rl = f"{champ_sp['roi_short']}/{champ_sp['roi_long']}"
    champ_rpt.print_full_report(
        f"OOS CHAMPION — ROI30 Momentum Gate"
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021–2024, ROI30d Momentum-of-Momentum Gate")
    print("=" * 72)
    ...
    print("  Iter 34 (BTC MVRV trend gate):  +2.55%/mo, Sharpe 1.00, MaxDD -26.42%")
    print(f"  Iter 35 (ROI30 mom gate):       {mo_is:+.2f}%/mo, Sharpe {sr_is:.2f}, MaxDD {dd_is:.2f}%")

    if n_pass >= 7:
        print("\n  *** 7/7 PASS — eligible for strategy report ***")
