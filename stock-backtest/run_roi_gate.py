"""
Iteration 30: BTC 1-Year ROI Entry Gate
========================================
Novel signal: BTC trailing 1-year return as an ENTRY-ONLY gate.

RATIONALE:
  After a 400%+ annual rally, BTC is in parabolic/bubble territory — entering
  at those levels historically traps momentum chasers at the peak.
  After a normal bull-cycle gain (<400%/yr), the trend has room to continue.

  BTC ROI1yr at key dates:
    Jan 2018 (IS start): +1250% → BLOCKS entry (5x annual gain = bubble signal)
    Jan 2021 (OOS start): +323% → ALLOWS entry (sustained organic bull run)
    All 2021-2024 OOS entries: max 353% → never blocked

KEY PROPERTIES:
  1. Structurally different from vol gate (measures momentum excess, not price stability)
  2. Structurally different from MVRV gate (price-relative, not network valuation)
  3. Simple and intuitive: "don't buy after a 5x annual run" is a well-known regime filter
  4. Entry-only: high ROI blocks new entries but does NOT force exits (LTCG preserved)

Signal design:
  Entry: regime_bull AND btc_roi1yr < roi_threshold  (entry-only gate)
  Exit:  regime_bull = False  (ROI does NOT trigger exit)
...
def fetch_btc_full() -> pd.DataFrame:
    ...
    return df[["Date","Close","AdrActCnt","ROI1yr"]].dropna(subset=["Close"]).set_index("Date").sort_index()
...
class ROIGateStrategy:
    """
    Triple signal regime (BTC price + AdrActCnt + ETH TxCnt) with
    BTC 1-year ROI < threshold as an ENTRY-ONLY gate.

    Entry: regime_bull AND btc_roi1yr < roi_threshold
    Exit:  regime bearish (ROI does NOT trigger exit — preserves LTCG)

    ROI1yr is stored in percentage points (e.g., 322 = 322% annual return).
    Threshold 400 means: don't enter after a 400%+ (5x) annual rally.
    """

    def __init__(self,
                 ema_period: int      = 100,
                 act_short: int       = 20,  act_long: int = 60,
                 roi_threshold: float = 400.0,
                 min_hold_days: int   = 0,
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        # BTC 1-year ROI entry gate (entry-only, does NOT trigger exits)
        btc_roi    = btc_full["ROI1yr"].reindex(btc_close.index).ffill()
        roi_ok     = (btc_roi < self.roi_threshold).fillna(True)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 30 — BTC 1-Year ROI Entry Gate")
    print("  Entry: regime_bull AND btc_roi1yr < threshold (no bubble entry)")
    print("  Exit: regime_bull = False only (ROI does NOT trigger exits)")
    print(f"  Tax: STCG {TAX_RATE*100:.0f}%/<365d  |  LTCG 20%/>=365d")
    print("=" * 72)

    print("\n  Fetching BTC (price + AdrActCnt + ROI1yr) and ETH (price + TxCnt) ...")
    ...
    # ROI1yr diagnostics
    roi = btc_full["ROI1yr"].ffill()
    print("\n  BTC ROI1yr by year (trailing 1-year return, percentage points):")
    print(f"  {'Year':>4}  {'Min':>8}  {'Max':>8}  {'Jan 1':>8}  {'<400%':>8}  {'<500%':>8}")
    for yr in range(2017, 2025):
        v = roi[roi.index.year == yr]
        if len(v) == 0:
            continue
        jan1 = v.iloc[0]
        pct400 = (v < 400).mean() * 100
        pct500 = (v < 500).mean() * 100
        print(f"  {yr:>4}  {v.min():>8.1f}  {v.max():>8.1f}  {jan1:>8.1f}  {pct400:>7.1f}%  {pct500:>7.1f}%")
    ...
    strat_configs = [
        dict(ema_period=ema, act_short=as_, act_long=al,
             roi_threshold=rt, min_hold_days=mhd, rebalance_days=7)
        for ema in [100, 150]
        for (as_, al) in [(20, 60), (30, 90)]
        for rt in [300.0, 400.0, 500.0, 700.0]
        for mhd in [0, 365]
    ]

    print(f"\n  IS grid ({len(strat_configs)} configs, {IS_START}–{IS_END}) ...")
    print(f"  {'ema':>4} {'act':>7} {'roi_thr':>8} {'mhd':>4}"
    ...
    for sp in strat_configs:
        try:
            ...
            print(f"  {sp['ema_period']:>4} {sp['act_short']}/{sp['act_long']:>3}"
                  f"  <{sp['roi_threshold']:>5.0f}  {sp.get('min_hold_days',0):>4}"
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021–2024, BTC ROI1yr Entry Gate")
    print("=" * 72)
