"""
Iteration 62: BTC Triple Regime — Price EMA + AdrActCnt + ETH TxCnt + XRP 5th Coin
===================================================================================
Strategy: Same proven ETH TxCnt triple regime from Iter 41, extended to 5-coin
portfolio replacing TRX with XRP.

WHY XRP over TRX as the 5th coin:
  TRX 2021: +180%   TRX 2022: -29%   TRX 2023: +97%   TRX 2024: +136%
  XRP 2021: +251%   XRP 2022: -60%   XRP 2023: +81%   XRP 2024: +230%

  XRP 2024 surge (+230%) driven by SEC lawsuit resolution (Jan 2025 settlement)
  → dramatic repricing of XRP fundamentals → significant alpha above TRX
  2022 drawdown XRP vs TRX: regime/vol-gate keeps strategy in cash during bear
  → 2022 drawdown difference should not materialize in portfolio equity

WHY SOL not viable:
  Coinmetrics SOL CSV only contains ~7 rows of price data (launches via
  ReferenceRateUSD only from late 2025). Insufficient for backtesting.

Regime signals (identical to Iter 41 champion):
  1. BTC price > EMA(ema_period)              — macro price trend
  2. BTC AdrActCnt EMA(short) > EMA(long)    — BTC network adoption
  3. ETH TxCnt EMA(short) > EMA(long)        — ETH economic activity (L2 anti-ATH)

Vol gate (entry-only, identical to Iter 41/Iter 60):
  Entry: regime_bull AND btc_vol30 < vol_threshold
  Exit:  regime_bull = False only (vol does NOT trigger exits → preserves LTCG)

ETH TxCnt Anti-ATH property:
  When BTC/ETH prices are high → gas high → users migrate to L2s → ETH mainnet
  TxCnt DECLINES → EMA trend turns BEARISH → regime OFF → no re-entries at peaks
  This structural property kept Iter 41 in CASH through ALL of 2022 bear market.
...
class EthTxCntXrpStrategy:
    """
    Triple regime (BTC price + BTC AdrActCnt + ETH TxCnt) with BTC vol gate.
    Entry: regime_bull AND btc_vol30 < vol_threshold
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        # 3. ETH TxCnt EMA trend (L2 anti-ATH mechanism)
        ...
        # Vol gate (entry-only, does NOT affect exits)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 62 — ETH TxCnt Triple Regime + Vol Gate + XRP (5-Coin)")
    print("  Portfolio: BTC, ETH, BNB, ADA, XRP  (20% each)")
    print("  Signal: BTC price EMA + BTC AdrActCnt + ETH TxCnt (anti-ATH L2)")
    ...
    # ETH TxCnt diagnostics (anti-ATH L2 migration mechanism)
    eth_tx   = eth_full["TxCnt"].ffill()
    tx_s20   = eth_tx.ewm(span=20, adjust=False).mean()
    tx_l60   = eth_tx.ewm(span=60, adjust=False).mean()
    tx_bull  = tx_s20 > tx_l60
    print("\n  ETH TxCnt EMA(20)>EMA(60) trend by year (anti-ATH L2 signal):")
    ...
    # ETH TxCnt Jan-Feb 2020 check (pre-COVID IS entry risk)
    jan20 = tx_bull[(tx_bull.index.year == 2020) & (tx_bull.index.month <= 2)]
    print(f"\n  ETH TxCnt Jan-Feb 2020 bull pct: {100*jan20.mean():.0f}%"
          f"  ({'SAFE <20%' if jan20.mean() < 0.20 else 'RISK: pre-COVID IS entries possible'})")
    ...
    print(f"\n  IS coins: {list(is_data.keys())}")
    print(f"  OOS coins: {list(combined_data.keys())}")
    ...
    ic_rpt.print_full_report(
        f"IS-CHAMPION OOS — ETH TxCnt Regime + XRP"
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021–2024, ETH TxCnt Regime + Vol Gate + XRP")
    print("=" * 72)
    ...
    if hd_ic >= 365 and eq_ic > 0:
        ...
        ltcg_adj = mo_ic * (1 - 0.20) / (1 - 0.35)
    else:
        ltcg_adj = mo_ic * (1 - 0.35)
