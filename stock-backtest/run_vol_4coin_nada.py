"""
Iteration 44: BTC Vol Gate — 4-Coin Portfolio (BTC, ETH, BNB, TRX — drops ADA)
=================================================================================
Tests whether dropping ADA (lowest OOS performer at +380%) and increasing
concentration in higher-OOS coins improves on the 5-coin portfolio (Iter 41).

PORTFOLIO EVOLUTION:
  Iter 27 (4-coin BTC/ETH/BNB/ADA, 25% each):  avg OOS  733% → +2.94%/mo
  Iter 41 (5-coin adds TRX, 20% each):           avg OOS  755% → +3.00%/mo
  Iter 44 (4-coin drops ADA, 25% each):           avg OOS  849% → ?

  OOS coin returns 2021-2024:
    BNB: +1751% (strongest — BSC DeFi)
    TRX: +844%  (strong — TRON stablecoin ecosystem)
    BTC: +400%  (moderate)
    ETH: +400%  (moderate)
    ADA: +380%  (weakest — dropped)

  By removing ADA (380% OOS) and redistributing its 20% weight to other coins
  (each goes from 20% to 25%), average OOS increases from 755% to 849%.

HYPOTHESIS:
  Dropping ADA should increase monthly return since:
  1. ADA is the lowest-OOS coin and was diluting the portfolio
  2. BNB and TRX get larger weights (25% vs 20%)
  3. We retain diversification with 4 different ecosystems (BTC/ETH/BSC/TRON)

SIGNAL (identical to Iter 41):
...
# ADA dropped: only BTC, ETH, BNB, TRX at 25% each
COINS = [
    ("btc", "BTC", 42),
    ("eth", "ETH", 11),
    ("bnb", "BNB", 77),
...
class VolGate4CoinNoADAStrategy:
    """
    BTC vol gate + 4-coin portfolio (BTC, ETH, BNB, TRX — no ADA).
    Entry: regime_bull AND btc_vol30 < vol_threshold
...
def run_is(full_data, is_data, btc_full, eth_full, sp, bt_fixed):
    strat = VolGate4CoinNoADAStrategy(**sp)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 44 — BTC Vol Gate + 4-Coin Portfolio (drops ADA)")
    print("  Portfolio: BTC 25% + ETH 25% + BNB 25% + TRX 25% (no ADA)")
    ...
    print("=" * 72)
    print("\n  OOS coin returns 2021-2024:")
    print("    BNB: +1751%  TRX: +844%  ETH: +400%  BTC: +400%  (ADA: +380% dropped)")
    print("  Avg OOS: (1751+844+400+400)/4 = 849% vs 5-coin 755%")
    ...
    champ_rpt.print_full_report(
        f"OOS CHAMPION — Vol Gate 4-Coin No-ADA"
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021–2024, Vol Gate + 4-Coin (BTC/ETH/BNB/TRX, no ADA)")
    print("=" * 72)
