"""
Iteration 53: BTC Realized Vol Gate — 6-Coin Portfolio (adds DOGE)
===================================================================
Extends Iter 41 (7/7 PASS, +3.00%/mo, +3.69%/mo after-LTCG) by adding DOGE
as a 6th coin. DOGE returned +5,571% OOS 2021-2024 vs ADA +380%, BNB +1,751%.

RATIONALE:
  Iter 41 IS-champion portfolio (BTC, ETH, BNB, ADA, TRX) average OOS: ~710%/coin
  DOGE OOS 2021-2024: +5,571% — driven by May 2021 Elon Musk pump (+12,000% peak)
  and a strong 2024 recovery. Even holding DOGE for full 2021-2024, buy-and-hold
  beats all existing 5 coins.

  DOGE IS 2018-2020: -46% (buy-and-hold). The triple regime (BTC price > EMA +
  BTC AdrActCnt + ETH TxCnt) turns BEARISH in early 2018 — blocking DOGE entry
  during the bear market and crash. Regime turns BULLISH in late 2020 (DeFi summer,
  BTC halving price action). DOGE entry late 2020 at ~$0.003 → Dec 2020 ~$0.006
  gives IS a small positive contribution without disrupting IS champion selection.

  Adding 1/6 DOGE allocation (16.7%) to the portfolio may boost net returns
  significantly while the regime filter protects against bear market drawdowns.

Portfolio: BTC, ETH, BNB, ADA, TRX, DOGE (6 coins, each 1/6 = 16.7%)
Triple regime (identical to Iter 41):
  1. BTC price > EMA(ema_period)
  2. BTC AdrActCnt EMA(short) > EMA(long)
  3. ETH TxCnt EMA(short) > EMA(long)
Vol Gate (entry-only): btc_vol30 < vol_threshold
...
class VolGate6CoinStrategy:
    """
    Triple signal regime (BTC price + AdrActCnt + ETH TxCnt) with
    BTC realized vol < threshold as an ENTRY-ONLY gate. 6-coin portfolio.
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 53 — BTC Vol Gate + 6-Coin Portfolio (adds DOGE)")
    ...
    # 6 positions, each 1/6 of capital
    bt_fixed = dict(
        max_positions=6,
        position_size_pct=0.167,
        atr_stop_multiplier=20.0,
        atr_trail_multiplier=12.0,
        risk_per_trade_pct=0.167,
    ...
    # Check for tie-breaking issues (IS score gap between mhd=365 and mhd=0)
    ...
    if mhd365_scores and mhd0_scores:
        ...
        print(f"\n  IS champion gap: mhd=365 best={best365:.2f}  mhd=0 best={best0:.2f}"
              f"  → {'mhd=365 WINS' if best365 > best0 else 'mhd=0 WINS (IS champion problem!)'}")
    ...
    ic_rpt.print_full_report(
        f"IS-CHAMPION OOS (walk-forward blind) — Vol Gate 6-Coin DOGE"
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021–2024, BTC Vol Gate + 6-Coin (DOGE added)")
    print("=" * 72)
