"""
Iteration 43: BTC Vol Gate — Concentrated Altcoin Portfolio (ETH, BNB, TRX)
=============================================================================
Key innovation: USE BTC vol gate as signal but EXCLUDE BTC from portfolio.
Concentrate in the 3 highest-OOS-return altcoins with proven on-chain fundamentals.

RATIONALE:
  In OOS 2021-2024, coins ranked by total return:
    BNB: +1751%  (BSC DeFi, highest OOS return)
    TRX: +844%   (TRON stablecoin ecosystem)
    ETH: +400%   (DeFi/NFT backbone)
    BTC: +400%   (lowest OOS among portfolio)
    ADA: +380%   (lowest OOS overall)

  5-coin equal-weight (Iter 41): avg OOS = (400+400+1751+844+380)/5 = 755%
  3-coin altcoin-only (Iter 43): avg OOS = (400+1751+844)/3 = 998%

  By dropping BTC and ADA (the two weakest OOS performers) and concentrating
  33% each in ETH, BNB, TRX, portfolio average OOS return improves by +32%.

  BTC remains as SIGNAL source (vol gate) but NOT a portfolio holding.
  This decouples the regime signal (BTC market dynamics) from the holdings.

PORTFOLIO:
  ETH (33.3%) + BNB (33.3%) + TRX (33.3%)
  max_positions=3, position_size_pct=0.33

SIGNAL (unchanged from Iter 41):
  Entry: regime_bull AND btc_vol30 < vol_threshold
  Exit:  regime_bull = False (vol does NOT trigger exits — preserves LTCG)

RISK CONSIDERATIONS:
  Higher concentration → higher vol → but also higher Sharpe from better entries
  3-coin portfolio still diversified across DeFi ecosystem (ETH), BSC (BNB), TRON (TRX)
  BTC exit signal preserves LTCG: positions held 365+ days
...
# Only altcoins in portfolio — BTC used as signal source, not held
COINS = [
    ("eth", "ETH", 11),
    ("bnb", "BNB", 77),
    ("trx", "TRX", 55),
...
class VolAltcoinStrategy:
    """
    BTC vol gate as entry signal. Only 3 altcoins held (ETH, BNB, TRX).
    BTC is NOT a portfolio position — it's the signal source only.
    Entry: regime_bull AND btc_vol30 < vol_threshold
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        # Triple regime (BTC price + AdrActCnt + ETH TxCnt)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 43 — BTC Vol Gate + Concentrated Altcoin Portfolio (ETH/BNB/TRX)")
    print("  BTC is signal-only: NOT held in portfolio")
    ...
    print("\n  Fetching BTC (signal: price + AdrActCnt) and ETH (price + TxCnt) ...")
    btc_full = fetch_btc_full()
    eth_full = fetch_eth_full()
    print(f"  BTC: {len(btc_full)} rows  last=${float(btc_full['Close'].iloc[-1]):,.2f}  [signal only]")
    print(f"  ETH: {len(eth_full)} rows  last=${float(eth_full['Close'].iloc[-1]):,.2f}")

    full_data = {}
    for coin, label, seed in COINS:
        try:
            s = eth_full["Close"] if label == "ETH" else fetch_close(coin)
    ...
    print("\n  BTC 30d realized vol (signal) at key dates:")
    for date_str, context in [
        ("2018-01-01", "IS bubble peak → BLOCK expected"),
        ("2020-10-01", "post-halving accumulation → ALLOW expected"),
        ("2021-01-01", "OOS start → ALLOW expected"),
        ("2021-06-01", "China ban crash → BLOCK expected"),
        ("2024-01-01", "ETF era → ALLOW expected"),
    ]:
        try:
            dt = pd.Timestamp(date_str)
            idx = btc_full.index.asof(dt)
            vl = float(vol30.loc[idx])
            print(f"  {date_str}: vol={vl:.2f}  {context}")
        except Exception:
            pass
    ...
    champ_rpt.print_full_report(
        f"OOS CHAMPION — Vol Gate Alt 3-Coin"
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021–2024, Vol Gate + 3-Coin Altcoin Portfolio")
    print("=" * 72)
