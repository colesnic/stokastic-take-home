"""
Iteration 27: BTC Realized Volatility Entry Gate
=================================================
Novel signal: BTC 30-day realized volatility as an ENTRY-ONLY gate.
...
KEY PROPERTY:
  BTC volatility was 126% in Jan 2018 → any threshold ≤ 100% blocks IS start entry.
  All OOS regime-entry points (early 2021, post-2022, post-2023) have vol < 80%.
  The gate is entry-only: once in position, exits are driven by regime signals
  (not vol spikes), preserving LTCG and avoiding false exits on short vol bursts.

Signal design:
  Entry: regime_bull AND btc_vol30 < vol_threshold  (entry-only gate)
  Exit:  regime_bull = False  (vol does NOT trigger exit)
...
class VolGateStrategy:
    """
    Triple signal regime (BTC price + AdrActCnt + ETH TxCnt) with
    BTC realized vol < threshold as an ENTRY-ONLY gate.
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        vol_ok    = (vol30 < self.vol_threshold).fillna(False)  # False until enough history
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 27 — BTC Realized Volatility Entry Gate")
    ...
    # IS-champion walk-forward blind result
    if is_champ_oos:
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021–2024, BTC Realized Vol Entry Gate")
    print("=" * 72)
