"""
Iteration 28: Combined Vol + MVRV Entry Gates
=============================================
Combines the two strongest novel entry gates:
  - Iter 27: BTC vol < threshold (no bubble entries)
  - Iter 25: ETH MVRV < threshold (no overvalued entries)

DESIGN:
  Entry: regime_bull AND btc_vol30 < vol_thr AND eth_mvrv < mvrv_thr
  Exit:  regime_bull = False (neither vol nor MVRV trigger exits)

HYPOTHESIS:
  Vol gate: prevents entering during explosive/bubble price action
  MVRV gate: prevents entering when ETH is overvalued vs realized cost
  Together: entries only in low-vol, fair-value windows → highest conviction
...
class VolMVRVGateStrategy:
    """
    Triple regime + both BTC vol AND ETH MVRV as entry-only gates.
    Entry: regime_bull AND vol30 < vol_thr AND mvrv < mvrv_thr
    Exit:  regime_bull = False
    """
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        entry_allowed = regime_bull & vol_ok & mvrv_ok
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 28 — Combined Vol + MVRV Entry Gates")
    print("  Entry: regime_bull AND vol30 < thr AND mvrv < thr")
    print("  Exit: regime_bull = False")
    ...
    print(f"  BTC: {len(btc_full)} rows  ETH: {len(eth_full)} rows")
    ...
    strat_configs = [
        dict(ema_period=ema, act_short=as_, act_long=al,
             vol_threshold=vt, mvrv_threshold=mv,
    ...
    print(f"  {'ema':>4} {'act':>7} {'vol':>6} {'mvrv':>5} {'mhd':>4}"
    ...
    for sp in strat_configs:
        try:
            ...
            print(f"  {sp['ema_period']:>4} {sp['act_short']}/{sp['act_long']:>3}"
                  f"  <{sp['vol_threshold']:.2f}  <{sp['mvrv_threshold']:.1f} {sp.get('min_hold_days',0):>4}"
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021–2024, Combined Vol + MVRV Gates")
    print("=" * 72)
