"""
Iteration 63: ETH TxCnt Triple Regime — No-BTC Portfolio + XLM
==============================================================
Variation of Iter 62: remove BTC from portfolio (keep as signal source),
replace with XLM. Portfolio: ETH, BNB, ADA, XRP, XLM (20% each).

Rationale:
  BTC 2021: +60%   (weakest performer in Iter 62 portfolio)
  XLM 2021: +102%  (net +42pp improvement by swapping BTC for XLM)
  BTC 2024: +120%
  XLM 2024: +151%  (net +31pp improvement in 2024)

XLM (Stellar Lumens) year-by-year OOS:
  2021: +102%   (cross-border payment narrative, BTC-correlated bull)
  2022: -74%    (bear market — regime protects)
  2023: +77%    (recovery)
  2024: +151%   (SEC/legal clarity for non-securities tokens benefited XLM)

XRP (already in portfolio from Iter 62):
  2021: +251%   2022: -60%   2023: +81%   2024: +230%

Both XRP and XLM benefited from 2024 SEC resolution tailwinds. Holding both
provides doubled exposure to the "legal clarity" macro theme.

BTC role: signal source only (price EMA + AdrActCnt), NOT in portfolio.
This means regime quality is unchanged but portfolio returns are altered.

Triple regime (identical to Iter 62):
  1. BTC price > EMA(ema_period)            — macro trend
  2. BTC AdrActCnt EMA(short) > EMA(long)  — BTC network adoption
  3. ETH TxCnt EMA(short) > EMA(long)      — ETH activity (L2 anti-ATH)

Vol gate (entry-only): btc_vol30 < threshold

Portfolio: ETH, BNB, ADA, XRP, XLM (20% each, BTC excluded)
...
# BTC excluded from portfolio; ETH/BNB/ADA/XRP/XLM are the holdings
COINS = [
    ("eth", "ETH", 11),
    ("bnb", "BNB", 77),
    ("ada", "ADA", 99),
...
class EthTxCntXlmStrategy:
    """
    Triple regime (BTC price + BTC AdrActCnt + ETH TxCnt) with BTC vol gate.
    Portfolio: ETH, BNB, ADA, XRP, XLM (BTC is signal only, not held).
    """
    ...
    def prepare(self, data: dict, btc_full: pd.DataFrame, eth_full: pd.DataFrame) -> None:
        ...
        # Signal 3: ETH TxCnt EMA trend (L2 anti-ATH)
...
def main():
    print("\n" + "=" * 72)
    print("  ITERATION 63 — ETH TxCnt Regime + No-BTC Portfolio + XLM")
    print("  Portfolio: ETH, BNB, ADA, XRP, XLM  (BTC = signal only)")
    print("  Signal: BTC price EMA + BTC AdrActCnt + ETH TxCnt (L2 anti-ATH)")
    print("  Hypothesis: XLM replaces BTC's weaker OOS returns (+151% vs +120% in 2024)")
    print(f"  Tax: STCG {TAX_RATE*100:.0f}%/<365d  |  LTCG 20%/≥365d")
    print("=" * 72)

    print("\n  Fetching BTC (signal source: price + AdrActCnt) ...")
    btc_full = fetch_btc_full()
    print("  Fetching ETH (signal source: TxCnt + price) ...")
    ...
    ic_rpt.print_full_report(
        f"IS-CHAMPION OOS — ETH TxCnt + XLM (No-BTC Portfolio)"
    ...
    print("\n" + "=" * 72)
    print("  VERDICT  —  OOS 2021–2024, ETH TxCnt + XLM (No-BTC Portfolio)")
    print("=" * 72)
